...
void set_up() {
    ...
    // The decode suite exercises the general 32-bit registers XWA-XHL, which
    // exist only in maximum mode. Every variant supports it (900/L resets to
    // maximum), so force it here; test_minmode switches back where needed.
...
void test_single() {
    ...
    // sub-byte 0x3C decodes as NSP/XNSP on every variant except /L (INTNEST, 16-bit only).
    if (is_tlcs900l()) {
    ...
    // Previous-bank (') general registers via the ABREG prefixes (C7=byte,
    // D7=word, E7=long); the register size must match the control register.
    TEST("LDC",   "DMAM0, A'",     0xC7, 0xD0, 0x2E, 0x22); // byte ctrl + byte reg
    TEST("LDC",   "DMAC0, WA'",    0xD7, 0xD0, 0x2E, 0x20); // word ctrl + word reg
    TEST("LDC",   "DMAS0, XWA'",   0xE7, 0xD0, 0x2E, 0x00); // long ctrl + long reg
    TEST("LDC",   "XWA', DMAS0",   0xE7, 0xD0, 0x2F, 0x00);
    TEST("EI",     "0", 0x06, 0x00);
    TEST("EI",     "7", 0x06, 0x07);
    ...
    // test_single runs in maximum mode (4 banks): LDF masks the value to 0-3.
...
void test_tlcs900l() {
    ...
    // The buffer must be 2^n (2*step <= size <= 0x8000); other encodings are
    // illegal operands (they would not round-trip through the assembler).
    ERRT("MINC1", "3, BC", ILLEGAL_OPERAND, "3, BC", 0xD9, 0x38, 0x02, 0x00); // not 2^n
    ERRT("MINC1", "1, BC", ILLEGAL_OPERAND, "1, BC", 0xD9, 0x38, 0x00, 0x00); // n=0, too small
    ERRT("MINC4", "4, BC", ILLEGAL_OPERAND, "4, BC", 0xD9, 0x3A, 0x00, 0x00); // n=2 < min n=3
    ERRT("MDEC1", "3, BC", ILLEGAL_OPERAND, "3, BC", 0xD9, 0x3C, 0x02, 0x00); // not 2^n
}
...
void test_ld8() {
    ...
    // LD r8, (mem): register-indirect single byte 0x80-0x87 = (XWA)..(XSP)
    ...
    // (XHL) byte-source: 0x83 is the block prefix, so the complex (r32) form
    TEST("LD", "A, (XHL)",       0xC3, 0xEC, 0x21);
    // (Xrr+d8) single byte 0x88-0x8F
    TEST("LD", "A, (XWA+32)",    0x88, 0x20, 0x21);
    TEST("LD", "A, (XIX+32)",    0x8C, 0x20, 0x21);
    TEST("LD", "A, (XIX-32)",    0x8C, 0xE0, 0x21);
    // pre-dec/post-inc: complex C4/C5 only (no single-byte form)
    ...
    // previous-bank / bank-N base registers (complex only)
    TEST("LD", "A, (XWA')",      0xC3, 0xD0, 0x21);
    TEST("LD", "A, (XBC0)",      0xC3, 0x04, 0x21);
    TEST("LD", "A, (XIX+1234H)", 0xC3, 0xF1, 0x34, 0x12, 0x21);
    TEST("LD", "A, (XIX-1234H)", 0xC3, 0xF1, 0xCC, 0xED, 0x21);
    ...
    // (XWA) destination: 0xB0 is the retcc prefix, so the complex form
    ...
    TEST("LD", "(XIX+), 34H",      0xF5, 0xF0, 0x00, 0x34);
...
void test_alu() {
    ...
    // ALU (mem), r -- the other direction, 88+r
    ...
    // LD (mem), #n8 (MEMD context); byte only, B0+mem:00:#
    TEST("LD", "(XIX), 34H",      0xB4, 0x00, 0x34);
    // LD (mem), (abs16)
...
void test_bit() {
    ...
    // Register bit ops are byte/word only (manual BW-); the 32-bit prefix is illegal.
    UNKN(0xE8, 0x30);
    UNKN(0xE9, 0x31);
...
void test_shift() {
    ...
    // Count 0 in the field means 16; a non-zero high nibble is illegal.
    TEST("RLC", "16, A", 0xC9, 0xE8, 0x00);
    ERRT("RLC", "A", ILLEGAL_OPERAND, "", 0xC9, 0xE8, 0xE9);
    // 16-bit register
    TEST("RLC", "1, BC", 0xD9, 0xE8, 0x01);
    TEST("SRL", "3, BC", 0xD9, 0xEF, 0x03);
    // On memory: single-shift only, 80+zz+mem:78-7F
...
void test_word_mem() {
    // <W> word memory forms decode distinctly from the byte forms.
    TEST("PUSH",  "(XIX)", 0x84, 0x04);
    ...
    TEST("ADDW",  "(XIX), 1234H", 0x94, 0x38, 0x34, 0x12);
    TEST("INCW",  "3, (XIX)", 0x94, 0x63);
    TEST("LDW",   "(XIX), 1234H", 0xB4, 0x02, 0x34, 0x12);
    // Long memory form (0xA4 prefix) is illegal for these byte/word ops.
    UNKN(0xA4, 0x04);
    UNKN(0xA4, 0x78);
}
...
void test_unary() {
    ...
    // NEG/CPL are byte/word only (manual B/W/L flag = BW-); the long form is
    // not a defined instruction.
    ERRT("", "", UNKNOWN_INSTRUCTION, "", 0xE8, 0x06);
    ERRT("", "", UNKNOWN_INSTRUCTION, "", 0xE8, 0x07);
    // DAA/EXTZ/EXTS/PAA/MIRR/MULA
    TEST("DAA",  "A",   0xC9, 0x10);
    TEST("EXTZ", "BC",  0xD9, 0x12);
    TEST("EXTZ", "XWA", 0xE8, 0x12);  // long form (no byte form exists)
    TEST("EXTS", "BC",  0xD9, 0x13);
    TEST("EXTS", "XWA", 0xE8, 0x13);
    ...
    // Non-WA destination (byte op: dest 16-bit) and word op (dest 32-bit).
    ...
    // Immediate source: dest carried in the prefix.
...
void test_branch() {
    ...
    // Block instructions: explicit operands. Byte (0x8x) / word <W> (0x9x);
    // the prefix low nibble is the source pointer (XHL=3 / XIY=5), so the
    // XDE/XHL pair is 0x83/0x93 and the XIX/XIY pair 0x85/0x95.
    TEST("LDI",   "(XDE+), (XHL+)", 0x83, 0x10);
    TEST("LDI",   "(XIX+), (XIY+)", 0x85, 0x10);
    TEST("LDIW",  "(XDE+), (XHL+)", 0x93, 0x10);
    TEST("LDIW",  "(XIX+), (XIY+)", 0x95, 0x10);
    TEST("LDIR",  "(XDE+), (XHL+)", 0x83, 0x11);
    TEST("LDIRW", "(XDE+), (XHL+)", 0x93, 0x11);
    TEST("LDD",   "(XDE-), (XHL-)", 0x83, 0x12);
    TEST("LDD",   "(XIX-), (XIY-)", 0x85, 0x12);
    TEST("LDDW",  "(XDE-), (XHL-)", 0x93, 0x12);
    TEST("LDDR",  "(XDE-), (XHL-)", 0x83, 0x13);
    TEST("LDDRW", "(XDE-), (XHL-)", 0x93, 0x13);
    TEST("CPI",  "A, (XHL+)", 0x83, 0x14);
    TEST("CPIR", "A, (XHL+)", 0x83, 0x15);
    TEST("CPD",  "A, (XHL-)", 0x83, 0x16);
    TEST("CPDR", "A, (XHL-)", 0x83, 0x17);
    // Block compare: any current-bank pointer, A (byte) / WA (word) accumulator.
    ...
    // Same prefix bytes as (XHL)/(XWA) memory ops -- opcode disambiguates.
...
void test_illegal() {
    ...
    // 0x83 is dual-use: (XHL) byte-source memory prefix and the block-transfer
    // prefix (opcodes 0x10-0x17). Opcodes defined for neither are illegal.
    UNKN(0x83, 0x00);
    UNKN(0x83, 0x05);
    UNKN(0x83, 0x18);
    UNKN(0x83, 0x28);

    // 0xB0 is dual-use: (XWA) destination memory prefix and the conditional
    // -return prefix (opcodes 0xF0-0xFF). Undefined MEMD opcodes are illegal.
    UNKN(0xB0, 0x01);
    UNKN(0xB0, 0x2F);
    ...
    // PM_MEMD (0xF3 + complex sub-byte): a sub-byte whose low 2 bits are 10
    // selects no defined addressing form, and a r32' code in 0x40-0xCC names a
    // non-existent register bank; both are illegal.
    UNKN(0xF3, 0x02);
    UNKN(0xF3, 0x40);
    // r32' code naming a non-existent bank is illegal in every complex slot:
    // source pre-dec (C4), dest pre-dec (F4), dest post-inc (F5).
    UNKN(0xC4, 0x40);
    UNKN(0xF4, 0x40);
    UNKN(0xF5, 0x40);

    // Size-restricted instructions decoded at the wrong register-prefix size.
    ERRT("", "", UNKNOWN_INSTRUCTION, "", 0xD9, 0x10); // DAA is byte only
    ERRT("", "", UNKNOWN_INSTRUCTION, "", 0xE9, 0x10);
    ERRT("", "", UNKNOWN_INSTRUCTION, "", 0xC9, 0x16); // MIRR is word only
    ERRT("", "", UNKNOWN_INSTRUCTION, "", 0xE9, 0x16);
    ERRT("", "", UNKNOWN_INSTRUCTION, "", 0xC9, 0x12); // EXTZ has no byte form
}

// clang-format on
// Minimum mode (TLCS900L): the general 32-bit registers XWA/XBC/XDE/XHL are
// unavailable (data operand and memory base); only XIX/XIY/XIZ/XSP remain.
void test_minmode() {
    // 900/L boots maximum mode; switch to minimum for the minimum-mode decodes.
    disassembler.setOption("maximum-mode", "off");
    // A prefix carrying a general 32-bit register (E8-EF) as a DATA operand is
    // rejected -- that register does not exist in minimum mode. A general
    // register used as a MEMORY base IS available; it is decoded with its
    // 16-bit name (WA/BC/DE/HL), while index registers stay 32-bit.
    UNKN(0xE9);                            // LD XWA, XBC: general 32-bit register operand
    TEST("LD",   "A, (HL)",   0x83, 0x21);       // general base -> 16-bit name
    TEST("LD",   "A, (-WA)",  0xC4, 0xE0, 0x21); // pre-decrement, 16-bit name
    TEST("LD",   "A, (HL+5)", 0x8B, 0x05, 0x21); // displacement, 16-bit name
    ...
    TEST("CPI",  "A, (XIX+)",      0x84, 0x14);
    // An opcode that promotes a 16-bit register prefix (D8-DF) to a 32-bit
    // operand is fully decoded, then flagged when the register is output.
    ERRT("MULA", "XWA",       UNKNOWN_INSTRUCTION, "", 0xD8, 0x19);
    // Index registers and 8/16-bit registers stay available in minimum mode.
    TEST("LD",   "A, B",          0xCA, 0x89);
    TEST("LD",   "WA, BC",        0xD9, 0x88);
    TEST("LD",   "A, (XIX)",      0x84, 0x21);
    TEST("LD",   "A, (-XIX)",     0xC4, 0xF0, 0x21);
    ...
    TEST("INC",  "1, A",          0xC9, 0x61);
    TEST("MUL",  "WA, B",         0xCA, 0x41);
    TEST("LDF",  "7",             0x17, 0x07);  // 8 register banks in minimum mode
}

void run_tests(const char *cpu) {
    disassembler.setCpu(cpu);
    if (is_tlcs900l()) {
        // /L is minimum-mode only: the maximum-mode decode suite does not apply.
