...
static void test_imm() {
    ...
    TEST(LWPI, "sym1234", ">1234", 0x02, 0xE0, 0x12, 0x34);
    TEST(LIMI, "sym89AB", ">89AB", 0x03, 0x00, 0x89, 0xAB);
}
...
static void test_reg_imm() {
    TEST(LI,   "R0,>0000",  "0",    0x02, 0x00, 0x00, 0x00);
    TEST(AI,   "R1,>0001",  "1",    0x02, 0x21, 0x00, 0x01);
    TEST(ANDI, "R8,>00FF",  "255",  0x02, 0x48, 0x00, 0xFF);
    TEST(ORI,  "R14,>FF00", "-256", 0x02, 0x6E, 0xFF, 0x00);
    TEST(CI,   "R15,>FFFF", "-1",   0x02, 0x8F, 0xFF, 0xFF);

    symtab.put(0x1234, "sym1234");

    TEST(LI,   "R2,sym1234", ">1234", 0x02, 0x02, 0x12, 0x34);
}
...
static void test_src() {
    ...
    TEST(DIVS, "@sym1234",     ">1234", 0x01, 0xA0, 0x12, 0x34);
    TEST(DIVS, "@sym1000(R4)", ">1000", 0x01, 0xA4, 0x10, 0x00);
    TEST(BLWP, "@sym9876",     ">9876", 0x04, 0x20, 0x98, 0x76);
    TEST(DEC,  "@neg2(R7)",      ">FFFE", 0x06, 0x27, 0xFF, 0xFE);
}
...
static void test_reg_src() {
    ...
    TEST(CZC, "@sym1234(R3),R7", ">1234", 0x25, 0xE3, 0x12, 0x34);
    TEST(XOR, "@offset2(R5),R4",   ">0002", 0x29, 0x25, 0x00, 0x02);
}
...
static void test_cnt_src() {
    ...
    TEST(STCR, "@offset2(R4),size7", ">0002,7",  0x35, 0xE4, 0x00, 0x02);
    TEST(STCR, "@offset2(R4),16",    ">0002",    0x34, 0x24, 0x00, 0x02);
    TEST(STCR, "@>1000(R4),size7",   "7",        0x35, 0xE4, 0x10, 0x00);
}
...
static void test_xop_src() {
    ...
    TEST(XOP,  "@sym9876,xop10",   ">9876,10", 0x2E, 0xA0, 0x98, 0x76);
    TEST(XOP,  "@sym9876(R1),8",   ">9876",    0x2E, 0x21, 0x98, 0x76);
    TEST(XOP,  "@>1234(R1),xop10", "10",       0x2E, 0xA1, 0x12, 0x34);
}
...
static void test_dst_src() {
    ...
    TEST(SZC,  "@sym1234(R10),@sym5678(R11)", ">1234 >5678", 0x4A, 0xEA, 0x12, 0x34, 0x56, 0x78);
    TEST(SZCB, "@sym1234,@sym5678",           ">1234 >5678", 0x58, 0x20, 0x12, 0x34, 0x56, 0x78);
    TEST(AB,   "R10,@sym4000(R11)",           ">4000", 0xBA, 0xCA, 0x40, 0x00);
    TEST(MOV,  "@zero(R10),@>0001(R11)",      ">0000", 0xCA, 0xEA, 0x00, 0x00, 0x00, 0x01);
    TEST(SOC,  "@sym1234,@sym5678(R11)",      ">1234 >5678", 0xEA, 0xE0, 0x12, 0x34, 0x56, 0x78);
    TEST(SOCB, "@sym1234(R10),@sym5678",      ">1234 >5678", 0xF8, 0x2A, 0x12, 0x34, 0x56, 0x78);
}

static void test_rel() {
    ATEST(0x1000, JMP, ">1002", "$+2",   0x10, 0x00);
    ATEST(0x1000, JLT, ">1000", "$+0",   0x11, 0xFF);
    ATEST(0x1000, JLE, ">1100", "$+256", 0x12, 0x7F);
    ATEST(0x1000, JEQ, ">0F02", "$-254", 0x13, 0x80);
    ATEST(0x1000, JHE, ">1004", "$+4",   0x14, 0x01);
    ATEST(0x1000, JGT, ">1006", "$+6",   0x15, 0x02);
    ATEST(0x1000, JNE, ">1008", "$+8",   0x16, 0x03);
    ATEST(0x1000, JNC, ">100A", "$+10",  0x17, 0x04);
    ATEST(0x1000, JOC, ">0FFE", "$-2",   0x18, 0xFE);
    ATEST(0x1000, JNO, ">0FFC", "$-4",   0x19, 0xFD);
    ATEST(0x1000, JL,  ">0FFA", "$-6",   0x1A, 0xFC);
    ATEST(0x1000, JH,  ">0FF8", "$-8",   0x1B, 0xFB);
    ATEST(0x1000, JOP, ">0FF6", "$-10",  0x1C, 0xFA);
    ...
    ATEST(0x1000, JMP, "sym1002", ">1002", 0x10, 0x00);
    ATEST(0x1000, JLT, "sym1000", ">1000", 0x11, 0xFF);
    ATEST(0x1000, JLE, "sym1100", ">1100", 0x12, 0x7F);
    ATEST(0x1000, JEQ, "sym0F02", ">0F02", 0x13, 0x80);
}

static void test_cru_off() {
    TEST(SBO, ">00", "R12+0",   0x1D, 0x00);
    TEST(SBZ, ">7F", "R12+127", 0x1E, 0x7F);
    TEST(TB,  ">80", "R12-128", 0x1F, 0x80);

    symtab.put(0x00, "zero");
    symtab.put(127, "off127");
    symtab.put(-128, "off_128");

    TEST(SBO, "zero",    ">00;R12+0",   0x1D, 0x00);
    TEST(SBZ, "off127",  ">7F;R12+127", 0x1E, 0x7F);
    TEST(TB,  "off_128", ">80;R12-128", 0x1F, 0x80);
}
