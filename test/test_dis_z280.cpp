...
void test_alu_8bit() {
    ...
    if (isZ280()) {
        TEST("ADD", "A, (1234H)",    0xDD, 0x87, 0x34, 0x12);
...
void test_illegal_z280() {
    static constexpr Config::opcode_t legal_ed[] = {
        ,
...
#if defined(LIBASM_Z280_EPU)
    static constexpr Config::opcode_t legal_ed_epu[] = {
        0x84, 0x85, 0x8C, 0x8D, 0x94, 0x95, 0x97, 0x9C,
        0x9D, 0x9F, 0xA4, 0xA5, 0xA6, 0xA7, 0xAC, 0xAD,
        0xAE, 0xAF, 0xB4, 0xB5, 0xBC, 0xBD,
    };
#endif
    for (auto i = 0; i < 0x100; i++) {
        const auto opc = static_cast<Config::opcode_t>(i);
        if (contains(ARRAY_RANGE(legal_ed), opc))
            continue;
#if defined(LIBASM_Z280_EPU)
        if (contains(ARRAY_RANGE(legal_ed_epu), opc))
            continue;
#endif
        UNKN(0xED, opc);
    }

    static constexpr Config::opcode_t legal_dd[] = {
        0x01, 0x03, 0x04, 0x05, 0x06, 0x09, 0x0B, 0x0C,
        0x0D, 0x0E, 0x11, 0x13, 0x14, 0x15, 0x16, 0x19,
        0x1B, 0x1C, 0x1D, 0x1E, 0x20, 0x21, 0x22, 0x23,
        0x24, 0x25, 0x26, 0x28, 0x29, 0x2A, 0x2B, 0x2C,
        0x2D, 0x2E, 0x31, 0x33, 0x34, 0x35, 0x36, 0x39,
        0x3B, 0x3C, 0x3D, 0x3E, 0x44, 0x45, 0x46, 0x4C,
        0x4D, 0x4E, 0x54, 0x55, 0x56, 0x5C, 0x5D, 0x5E,
        ...
        0xC4, 0xC5, 0xCA, 0xCB, 0xCC, 0xCD, 0xD1, 0xD2,
        0xD4, 0xD5, 0xDA, 0xDC, 0xE1, 0xE2, 0xE3, 0xE4,
        0xE5, 0xE9, 0xEA, 0xEB, 0xEC, 0xED, 0xF1, 0xF2,
        0xF4, 0xF5, 0xF9, 0xFA, 0xFC,
    };
    for (auto i = 0; i < 0x100; i++) {
        const auto opc = static_cast<Config::opcode_t>(i);
        if (!contains(ARRAY_RANGE(legal_dd), opc))
            UNKN(0xDD, opc);
    }

    static constexpr Config::opcode_t legal_fd[] = {
        ,
    };
    for (auto i = 0; i < 0x100; i++) {
        const auto opc = static_cast<Config::opcode_t>(i);
        if (!contains(ARRAY_RANGE(legal_fd), opc))
            UNKN(0xFD, opc);
    }

    static constexpr Config::opcode_t legal_dded[] = {
        0x02, 0x04, 0x05, 0x06, 0x07, 0x0A, 0x0C, 0x0D,
        0x0E, 0x0F, 0x12, 0x14, 0x15, 0x16, 0x17, 0x1A,
        0x1C, 0x1D, 0x1E, 0x1F, 0x22, 0x24, 0x25, 0x26,
        0x27, 0x2A, 0x2C, 0x2D, 0x2E, 0x2F, 0x32, 0x34,
        0x35, 0x36, 0x37, 0x3A, 0x3C, 0x3D, 0x3E, 0x3F,
        0x40, 0x41, 0x42, 0x48, 0x49, 0x4A, 0x50, 0x51,
        0x52, 0x58, 0x59, 0x5A, 0x60, 0x61, 0x62, 0x66,
        0x68, 0x69, 0x6A, 0x6D, 0x6E, 0x72, 0x78, 0x79,
        0x7A, 0x86, 0x87, 0x8E, 0x8F, 0x96, 0x9E, 0xC0,
    ...
    for (auto i = 0; i < 0x100; i++) {
        const auto opc = static_cast<Config::opcode_t>(i);
        if (!contains(ARRAY_RANGE(legal_dded), opc))
            UNKN(0xDD, 0xED, opc);
    }

    static constexpr Config::opcode_t legal_fded[] = {
        ,
    };
    for (auto i = 0; i < 0x100; i++) {
        const auto opc = static_cast<Config::opcode_t>(i);
        if (!contains(ARRAY_RANGE(legal_fded), opc))
            UNKN(0xFD, 0xED, opc);
    }

    // DDCB / FDCB: Z80 base set plus TSET (0x36).
    static constexpr Config::opcode_t legal_ixbit_z280[] = {
    ...
    for (auto i = 0; i < 0x100; i++) {
        const auto opc = static_cast<Config::opcode_t>(i);
        if (contains(ARRAY_RANGE(legal_ixbit_z280), opc))
            continue;
