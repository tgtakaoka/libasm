...
static void test_cpu() {
    ...
    asserter.equals(
        "cpu Z88C00", true, disassembler.setCpu("Z88C00"));
    asserter.equals(
        "get cpu", "Z88", disassembler.getCpu());

    asserter.equals(
        "cpu Super8", true, disassembler.setCpu("Super8"));
    asserter.equals(
        "get cpu", "Z88", disassembler.getCpu());
}
...
static void test_one_operand() {
    ...
    TEST(SRP,  "#38H",  0x31, 0x38);
    TEST(SRP0, "#38H",  0x31, 0x3A);
...
static void test_two_operands() {
    ...
    TEST(LD, "R6,R5",      0xE4, 0xC5, 0xC6);
    TEST(LD, "R7,@R6",     0xE5, 0xC6, 0xC7);
    TEST(LD, "R7,#0E8H",   0xE6, 0xC7, 0xE8);
...
static void assert_illegal(uint8_t opc) {
    ...
    asserter.equals(message, UNKNOWN_INSTRUCTION, disassembler);
}
