...
static void test_rel() {
    if (w65c816()) {
        // W65C816
        ATEST(0x1000, BPL, "$001002", 0x10, 0x00);
        ATEST(0x1000, BMI, "$001000", 0x30, 0xFE);
        ATEST(0x1000, BVC, "$001004", 0x50, 0x02);
        ATEST(0x1000, BVS, "$001081", 0x70, 0x7F);
        ATEST(0x1000, BCC, "$000F82", 0x90, 0x80);
        ATEST(0x1000, BCS, "$001002", 0xB0, 0x00);
        ATEST(0x1000, BNE, "$001002", 0xD0, 0x00);
        ATEST(0x1000, BEQ, "$001002", 0xF0, 0x00);
    } else {
        // MOS6502
...
int main(int argc, char **argv) {
    ...
    for (size_t i = 0; i < sizeof(cpus)/sizeof(cpus[0]); i++) {
        ...
        if (w65c816()) RUN_TEST(test_zpg_long);
        if (w65c816()) RUN_TEST(test_sp_rel);
        RUN_TEST(test_abs);
        if (w65c816()) RUN_TEST(test_abs_long);
        ...
        if (w65sc02()) RUN_TEST(test_illegal_w65sc02);
