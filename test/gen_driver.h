...
public:
    GenDriver(Disassembler &disassembler)
        : _disassembler(disassembler), _listing() {}
    ...
    int main(int argc, const char **argv) {
        ...
        if (_output_name) {
            ...
            printCommandLine(_output, commentChar, _progname, argc, argv);
        }
        _list = nullptr;
        if (_list_name) {
...
private:
    ...
    void printCommandLine(FILE *out, char commentChar, const char *progname,
            int argc, const char **argv) {
        fprintf(out, "%c%c%c AUTO GENERATED FILE\n", commentChar, commentChar,
                commentChar);
        fprintf(out, "%c%c%c generated by: %s", commentChar, commentChar,
                commentChar, progname);
        for (int i = 1; i < argc; i++)
