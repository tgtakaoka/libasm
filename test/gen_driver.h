...
private:
    ...
    int parseOption(int argc, const char **argv) {
        ...
        if (_cpu == nullptr) _cpu = _disassembler.getCpu();
        return 0;
    }
    ...
    void printCommandLine(
            FILE *out, char commentChar, const char *progname, int argc, const char **argv) {
        fprintf(out, "%c%c%c AUTO GENERATED FILE\n", commentChar, commentChar, commentChar);
        fprintf(out, "%c%c%c generated by: %s", commentChar, commentChar, commentChar, progname);
