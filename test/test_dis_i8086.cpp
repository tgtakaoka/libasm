...
bool is80486() {
    return strcmp_P("80486", disassembler.config().cpu_P()) == 0;
}
...
bool is80387() {
    return strcmp_P("80387", dis8086.fpu_P()) == 0 || strcmp_P("80487", dis8086.fpu_P()) == 0
        || strcmp_P("80C187", dis8086.fpu_P()) == 0;
}
...
bool fpu_on() {
    if (is8086()) {
        ...
        EQUALS("80486", OK, dis8086.setFpuType(FPU_ON));
        EQUALS_P("80487", "80487", dis8086.fpu_P());
    } else {
...
void ptrSuffix(bool enable) {
    dis8086.setOption("ptr-suffix", enable ? "on" : "off");
}
...
void test_cpu() {
    ...
    EQUALS("cpu 80486", true,   disassembler.setCpu("80486"));
    EQUALS_P("cpu 80486", "80486", disassembler.config().cpu_P());

    EQUALS("cpu i80486", true,   disassembler.setCpu("i80486"));
    EQUALS_P("cpu i80486", "80486", disassembler.config().cpu_P());
...
void test_logic() {
    ...
    if (v30()) {
        ...
        TEST("TEST1", "CH, 1",                 0x0F, 0x18, 0305, 0x01);
        TEST("TEST1", "BYTE [SI], 2",          0x0F, 0x18, 0004, 0x02);
        TEST("TEST1", "BYTE [BX+DI+52], 7",    0x0F, 0x18, 0101, 0x34, 0x07);
        TEST("TEST1", "BP, 1",                 0x0F, 0x19, 0305, 0x01);
        TEST("TEST1", "WORD [SI], 3",          0x0F, 0x19, 0004, 0x03);
        TEST("TEST1", "CH, CL",                0x0F, 0x10, 0305);
        TEST("TEST1", "BYTE [SI], CL",         0x0F, 0x10, 0004);
        TEST("TEST1", "BP, CL",                0x0F, 0x11, 0305);
        TEST("TEST1", "WORD [SI], CL",         0x0F, 0x11, 0004);
    }
}
...
void test_repeat() {
    ...
    if (is80386() || is80486()) {
        ...
        // In use32 the roles invert: INSW/OUTSW (16-bit) carry the operand-size
        // prefix, INSD/OUTSD (32-bit) are native. (Round-trips the assembler.)
        use32();
        TEST("INSW",  "", DATA32, 0x6D);
        TEST("OUTSW", "", DATA32, 0x6F);
        TEST("INSD",  "", 0x6D);
        TEST("OUTSD", "", 0x6F);
        use16();
    }
...
void test_lock() {
    ...
    if (is80386() || is80486()) {
        ...
        TEST("LOCK XCHG", "[1234H], EDX",  LOCK, DATA32, 0x87, 0026, 0x34, 0x12);
    }

    TEST("LOCK XCHG", "[BX], AL",              LOCK, 0x86, 0007);
    TEST("LOCK ADD", "BYTE PTR [SI], 1",       LOCK, 0x80, 0004, 1);
    TEST("LOCK OR",  "[DI], AX",               LOCK, 0x09, 0x05);
    TEST("LOCK ADC", "BYTE PTR [BP+SI+1234H], 5", LOCK, 0x80, 0222, 0x34, 0x12, 5);
    TEST("LOCK SBB", "[BX+DI-52], DX",         LOCK, 0x19, 0121, 0xCC);
    TEST("LOCK AND", "BYTE PTR [SI+1234H], 255",  LOCK, 0x80, 0244, 0x34, 0x12, 0xFF);
    TEST("LOCK SUB", "[BX], SI",               LOCK, 0x29, 0067);
    TEST("LOCK XOR", "[BP+52], CL",            LOCK, 0x30, 0x4E, 0x34);
    ERRT("LOCK CMP", "WORD PTR [SI], 1234H",
         ILLEGAL_COMBINATION, "CMP",          LOCK, 0x81, 0074, 0x34, 0x12);
    TEST("LOCK NOT", "BYTE PTR [DI]",          LOCK, 0xF6, 0025);
    TEST("LOCK NEG", "WORD PTR [BX+SI]",       LOCK, 0xF7, 0030);
    TEST("LOCK INC", "BYTE PTR [BX+DI+52]",    LOCK, 0xFE, 0101, 0x34);
    TEST("LOCK DEC", "WORD PTR [BP+SI+1234H]", LOCK, 0xFF, 0212, 0x34, 0x12);

    ERRT("LOCK MOV", "[BX], AL", ILLEGAL_COMBINATION, "MOV",  LOCK, 0x88, 0007);
    ERRT("LOCK ADD", "AL, 1",    ILLEGAL_COMBINATION, "ADD",  LOCK, 0x04, 0001);
    ERRT("LOCK PUSH", "AX",      ILLEGAL_COMBINATION, "PUSH", LOCK, 0120);
    ERRT("", "", OPCODE_HAS_NO_EFFECT, "", LOCK, LOCK);

    TEST("LOCK ADD", "ES:[BX], AL", LOCK, 0x26, 0x00, 0007);
    ...
    TEST("LOCK ADD", "BYTE PTR [BP+SI+1234H], 86", LOCK, 0x80, 0202, 0x34, 0x12, 0x56);
    ...
    TEST("LOCK ADD", "WORD PTR [1234H], 86",  LOCK, 0x83, 0006, 0x34, 0x12, 0x56);
    ERRT("LOCK ADD", "AL, 86",    ILLEGAL_COMBINATION, "ADD", LOCK, 0x04, 0x56);
    ...
    TEST("LOCK ADD", "BYTE PTR [SI], 204", LOCK, 0x80, 0004, 0xCC);
    TEST("LOCK OR",  "BYTE PTR [SI], 204", LOCK, 0x80, 0014, 0xCC);
    TEST("LOCK ADC", "BYTE PTR [SI], 204", LOCK, 0x80, 0024, 0xCC);
    TEST("LOCK SBB", "BYTE PTR [SI], 204", LOCK, 0x80, 0034, 0xCC);
    TEST("LOCK AND", "BYTE PTR [SI], 204", LOCK, 0x80, 0044, 0xCC);
    TEST("LOCK SUB", "BYTE PTR [SI], 204", LOCK, 0x80, 0054, 0xCC);
    TEST("LOCK XOR", "BYTE PTR [SI], 204", LOCK, 0x80, 0064, 0xCC);
    ERRT("LOCK CMP", "BYTE PTR [SI], 204",
         ILLEGAL_COMBINATION, "CMP",       LOCK, 0x80, 0074, 0xCC);
    TEST("LOCK ADD", "WORD PTR [SI], -52", LOCK, 0x83, 0004, 0xCC);
    TEST("LOCK OR",  "WORD PTR [SI], -52", LOCK, 0x83, 0014, 0xCC);
    TEST("LOCK ADC", "WORD PTR [SI], -52", LOCK, 0x83, 0024, 0xCC);
    TEST("LOCK SBB", "WORD PTR [SI], -52", LOCK, 0x83, 0034, 0xCC);
    TEST("LOCK AND", "WORD PTR [SI], -52", LOCK, 0x83, 0044, 0xCC);
    TEST("LOCK SUB", "WORD PTR [SI], -52", LOCK, 0x83, 0054, 0xCC);
    TEST("LOCK XOR", "WORD PTR [SI], -52", LOCK, 0x83, 0064, 0xCC);
    ERRT("LOCK CMP", "WORD PTR [SI], -52",
...
void test_control_transfer() {
    if (is80386() || is80486()) {
        ...
        // CALL/JMP rel16 in use32: the 0x66 byte narrows the operand to
        // 16-bit. It is named DATA16 (a "DATA32" would be redundant in
        // use32) and must be explicit since a 16-bit target is also reachable
        // by the default 32-bit form, so the value alone cannot recover it.
        use32();
        ATEST(0x00001000, "DATA16 CALL", "00002238H", DATA32, 0xE8, 0x34, 0x12);
        ATEST(0x00001000, "DATA16 JMP",  "00002239H", DATA32, 0xE9, 0x35, 0x12);
        use16();
    } else if (is80286()) {
        // CALL rel16
    ...
    if (is80386() || is80486()) {
        ...
        use32();
        TEST("LOOP",   "$, ECX",         0xE2, 0xFE);
        TEST("LOOPE",  "$, ECX",         0xE1, 0xFE);
        TEST("LOOPNE", "$, ECX",         0xE0, 0xFE);
        TEST("LOOP",   "$, CX",  ADDR16, 0xE2, 0xFD);
        TEST("LOOPE",  "$, CX",  ADDR16, 0xE1, 0xFD);
        TEST("LOOPNE", "$, CX",  ADDR16, 0xE0, 0xFD);
        EPRE("LOOP",   "$, ECX", DATA16, 0xE2, 0xFD);
        EPRE("LOOPE",  "$, ECX", DATA16, 0xE1, 0xFD);
        EPRE("LOOPNE", "$, ECX", DATA16, 0xE0, 0xFD);
        EPRE("LOOP",   "$, CX",  ADDR16, DATA16, 0xE2, 0xFC);
        EPRE("LOOPE",  "$, CX",  DATA16, ADDR16, 0xE1, 0xFC);
        EPRE("LOOPNE", "$, CX",  ADDR16, DATA16, 0xE0, 0xFC);
        use16();
    }
    ...
    if (is80386() || is80486()) {
        ...
        // CALL/JMP rel16 in use32: 0x66 narrows the operand to 16-bit, named
        // data16. GAS rejects a redundant "data32" prefix in .code32.
        use32();
        ATEST(0x00001000, "data16 call", "0x00002238", DATA32, 0xE8, 0x34, 0x12);
        ATEST(0x00001000, "data16 jmp",  "0x00002239", DATA32, 0xE9, 0x35, 0x12);
        use16();
    }
...
void test_processor_control() {
    ...
    if (is80386() || is80486()) {
        ...
        if (is80486()) {
            // i80486 adds the cache-test registers TR3-TR5.
            TEST("MOV", "EDX, TR3", 0x0F, 0x24, 0332);
            TEST("MOV", "ECX, TR4", 0x0F, 0x24, 0341);
            TEST("MOV", "EAX, TR5", 0x0F, 0x24, 0350);
        } else {
        ...
        if (is80486()) {
            // i80486 adds the cache-test registers TR3-TR5.
            TEST("MOV", "TR3, EAX", 0x0F, 0x26, 0330);
            TEST("MOV", "TR4, EDI", 0x0F, 0x26, 0347);
            TEST("MOV", "TR5, ESI", 0x0F, 0x26, 0356);
        } else {
...
void test_bit_manipulation() {
    ...
    // BTC r/m16,r16
    TEST("BTC", "SP, AX",            0x0F, 0xBB, 0304);
    TEST("BTC", "[BX+SI], AX",       0x0F, 0xBB, 0000);
    TEST("BTC", "[1234H], SI",       0x0F, 0xBB, 0066, 0x34, 0x12);
    TEST("BTC", "[SI], SP",          0x0F, 0xBB, 0044);
    TEST("BTC", "[DI+52], BP",       0x0F, 0xBB, 0155, 0x34);
    TEST("BTC", "[BX+DI+52], CX",    0x0F, 0xBB, 0111, 0x34);
    TEST("BTC", "[BP+1234H], SI",    0x0F, 0xBB, 0266, 0x34, 0x12);
    TEST("BTC", "[BP+SI+1234H], DX", 0x0F, 0xBB, 0222, 0x34, 0x12);
    // BTC r/m32,r32
    TEST("BTC", "ESP, EAX",           DATA32, 0x0F, 0xBB, 0304);
    TEST("BTC", "[BX+SI], EAX",       DATA32, 0x0F, 0xBB, 0000);
    TEST("BTC", "[1234H], ESI",       DATA32, 0x0F, 0xBB, 0066, 0x34, 0x12);
    TEST("BTC", "[SI], ESP",          DATA32, 0x0F, 0xBB, 0044);
    TEST("BTC", "[DI+52], EBP",       DATA32, 0x0F, 0xBB, 0155, 0x34);
    TEST("BTC", "[BX+DI+52], ECX",    DATA32, 0x0F, 0xBB, 0111, 0x34);
    TEST("BTC", "[BP+1234H], ESI",    DATA32, 0x0F, 0xBB, 0266, 0x34, 0x12);
    TEST("BTC", "[BP+SI+1234H], EDX", DATA32, 0x0F, 0xBB, 0222, 0x34, 0x12);
...
void test_32bit_addressing() {
    ...
    // In 32-bit mode a base/index displacement is a signed integer: a
    // high-bit-set value is shown negative (so it reassembles minimally),
    // a positive value keeps its magnitude. (use16 keeps the raw word.)
    gnu_as(false);
    use32();
    // disp32 (native 32-bit addressing)
    TEST("XOR", "EAX, [EAX-3E3F4042H]", 0x33, 0200, 0xBE, 0xBF, 0xC0, 0xC1);
    TEST("XOR", "EAX, [EAX+12345678H]", 0x33, 0200, 0x78, 0x56, 0x34, 0x12);
    TEST("XOR", "EAX, [EAX-2]",         0x33, 0200, 0xFE, 0xFF, 0xFF, 0xFF);
    // disp16 (16-bit addressing via ADDR32 override in 32-bit mode)
    TEST("XOR", "AX, [BX+SI-2]",        ADDR32, 0x66, 0x33, 0200, 0xFE, 0xFF);
    TEST("XOR", "AX, [BX+SI+1234H]",    ADDR32, 0x66, 0x33, 0200, 0x34, 0x12);
    // moffs direct address: the offset width follows the address size, so it
    // is 32-bit in use32 (native) and 16-bit with an ADDR32 16-bit override.
    TEST("MOV", "AL, [29282726H]",  0xA0, 0x26, 0x27, 0x28, 0x29);
    TEST("MOV", "[29282726H], AL",  0xA2, 0x26, 0x27, 0x28, 0x29);
    TEST("MOV", "EAX, [29282726H]", 0xA1, 0x26, 0x27, 0x28, 0x29);
    TEST("MOV", "[29282726H], EAX", 0xA3, 0x26, 0x27, 0x28, 0x29);
    TEST("MOV", "AL, [2726H]",      ADDR32, 0xA0, 0x26, 0x27);
    TEST("MOV", "AX, [2726H]",      ADDR32, 0x66, 0xA1, 0x26, 0x27);
    use16();
}
...
void test_float() {
    ...
    if (is80386()) {
        // FPU no-operand instructions (opcodes E0-E2 under DB escape) must not
        // get the loop-counter register suffix in gnu-as mode on 32-bit CPUs.
        gnu_as(true);
        TEST("feni",  "", FWAIT, 0xDB, 0xE0);
        TEST("fdisi", "", FWAIT, 0xDB, 0xE1);
        TEST("fclex", "", FWAIT, 0xDB, 0xE2);
...
void test_illegal_80386() {
    ...
    static constexpr Config::opcode_t LEGALS_0F[] = {
        // inherited from 80286
        0x00, 0x01, 0x02, 0x03, 0x06,
        // CR/DR/TR register moves (0x25 is not valid)
        ...
        // PUSH/POP FS/GS, BT, SHLD, SHRD, IMUL, BTS, BTR, BTC, BSF, BSR
        0xA0, 0xA1, 0xA3, 0xA4, 0xA5,
        0xA8, 0xA9, 0xAB, 0xAC, 0xAD, 0xAF,
        // LSS/LFS/LGS, BTR, BTC, BSF, BSR, MOVZX, MOVSX
        0xB2, 0xB3, 0xB4, 0xB5, 0xB6, 0xB7,
        0xBB, 0xBC, 0xBD, 0xBE, 0xBF,
        0xBA,  // BT/BTS/BTR/BTC with immediate
    };
    for (auto i = 0; i < 0x100; i++) {
        const Config::opcode_t opc = i;
        if (contains(ARRAY_RANGE(LEGALS_0F), opc))
...
void test_i80486() {
    // BSWAP r32 (0F C8+rd) - use16 mode without DATA32 (permissive; decoded as r32)
    TEST("BSWAP", "EAX", 0x0F, 0xC8);
    TEST("BSWAP", "EDI", 0x0F, 0xCF);
    // BSWAP r32 with DATA32 prefix (spec-required encoding in use16 mode)
    ...
    // BSWAP r32 in use32 mode - opcode alone (DATA32 not needed)
    use32();
    TEST("BSWAP", "EAX", 0x0F, 0xC8);
    TEST("BSWAP", "EDI", 0x0F, 0xCF);
    use16();
    ...
    TEST("XADD", "CL, DL",         0x0F, 0xC0, 0321);
    TEST("XADD", "[BX+SI], CL",    0x0F, 0xC0, 0010);
    TEST("XADD", "[BX+DI], DL",    0x0F, 0xC0, 0021);
    TEST("XADD", "[SI+52], BL",    0x0F, 0xC0, 0134, 0x34);
    TEST("XADD", "[BX+1234H], CL", 0x0F, 0xC0, 0217, 0x34, 0x12);
    TEST("XADD", "[1234H], DL",    0x0F, 0xC0, 0026, 0x34, 0x12);
    // XADD r/m16, r16
    TEST("XADD", "AX, BX",         0x0F, 0xC1, 0330);
    TEST("XADD", "CX, DX",         0x0F, 0xC1, 0321);
    TEST("XADD", "[BX+SI], CX",    0x0F, 0xC1, 0010);
    TEST("XADD", "[BX], AX",       0x0F, 0xC1, 0007);
    TEST("XADD", "[BX+52], BX",    0x0F, 0xC1, 0137, 0x34);
    TEST("XADD", "[1234H], DX",    0x0F, 0xC1, 0026, 0x34, 0x12);
    // XADD r/m32, r32
    TEST("XADD", "EAX, EBX",       DATA32, 0x0F, 0xC1, 0330);
    TEST("XADD", "ECX, EDX",       DATA32, 0x0F, 0xC1, 0321);
    TEST("XADD", "[BX+SI], ECX",   DATA32, 0x0F, 0xC1, 0010);
    TEST("XADD", "[BX], EAX",      DATA32, 0x0F, 0xC1, 0007);
    TEST("XADD", "[1234H], EDX",   DATA32, 0x0F, 0xC1, 0026, 0x34, 0x12);
    // CMPXCHG r/m8, r8
    TEST("CMPXCHG", "AL, BL",         0x0F, 0xB0, 0330);
    TEST("CMPXCHG", "[BX+SI], CL",    0x0F, 0xB0, 0010);
    TEST("CMPXCHG", "[BX], DL",       0x0F, 0xB0, 0027);
    TEST("CMPXCHG", "[BX+52], BL",    0x0F, 0xB0, 0137, 0x34);
    TEST("CMPXCHG", "[1234H], CL",    0x0F, 0xB0, 0016, 0x34, 0x12);
    // CMPXCHG r/m16, r16
    TEST("CMPXCHG", "AX, BX",         0x0F, 0xB1, 0330);
    TEST("CMPXCHG", "[BX+SI], CX",    0x0F, 0xB1, 0010);
    TEST("CMPXCHG", "[BX], DX",       0x0F, 0xB1, 0027);
    TEST("CMPXCHG", "[1234H], BX",    0x0F, 0xB1, 0036, 0x34, 0x12);
    // CMPXCHG r/m32, r32
    TEST("CMPXCHG", "EAX, EBX",       DATA32, 0x0F, 0xB1, 0330);
    TEST("CMPXCHG", "[BX+SI], ECX",   DATA32, 0x0F, 0xB1, 0010);
    TEST("CMPXCHG", "[BX], EDX",      DATA32, 0x0F, 0xB1, 0027);
    TEST("CMPXCHG", "[1234H], EBX",   DATA32, 0x0F, 0xB1, 0036, 0x34, 0x12);
    // INVD / WBINVD
    TEST("INVD",   "",  0x0F, 0x08);
    TEST("WBINVD", "",  0x0F, 0x09);
    // INVLPG m (memory only, mod != 3)
    TEST("INVLPG", "[BX+SI]",       0x0F, 0x01, 0070);
    TEST("INVLPG", "[BX+DI]",       0x0F, 0x01, 0071);
    TEST("INVLPG", "[SI]",          0x0F, 0x01, 0074);
    TEST("INVLPG", "[BX]",          0x0F, 0x01, 0077);
    TEST("INVLPG", "[BX+52]",       0x0F, 0x01, 0177, 0x34);
    TEST("INVLPG", "[BP+1234H]",    0x0F, 0x01, 0276, 0x34, 0x12);
    TEST("INVLPG", "[1234H]",       0x0F, 0x01, 0076, 0x34, 0x12);
    // LOCK XADD / LOCK CMPXCHG
    lockInsn(false);
    TEST("LOCK XADD",    "[BX+SI], CL",   LOCK, 0x0F, 0xC0, 0010);
    TEST("LOCK XADD",    "[BX+SI], CX",   LOCK, 0x0F, 0xC1, 0010);
    TEST("LOCK XADD",    "[BX+SI], ECX",  LOCK, DATA32, 0x0F, 0xC1, 0010);
    TEST("LOCK CMPXCHG", "[BX+SI], CL",   LOCK, 0x0F, 0xB0, 0010);
    TEST("LOCK CMPXCHG", "[BX+SI], CX",   LOCK, 0x0F, 0xB1, 0010);
    TEST("LOCK CMPXCHG", "[BX+SI], ECX",  LOCK, DATA32, 0x0F, 0xB1, 0010);
}

void test_illegal_80486() {
    static constexpr Config::opcode_t ILLEGALS[] = {
        0x82, 0xD6, 0xF1,
    };
    for (const auto opc : ILLEGALS)
    ...
    static constexpr Config::opcode_t LEGALS_0F[] = {
        ...
        0xBA,  // BT/BTS/BTR/BTC with immediate
        // i80486 new
        0x08, 0x09,                          // INVD, WBINVD
        0xB0, 0xB1,                          // CMPXCHG byte/word
        0xC0, 0xC1,                          // XADD byte/word
        ...
        for (auto reg = 0; reg < 8; reg++) {
            for (auto r_m = 0; r_m < 8; r_m++) {
                ...
                if (mod == 3) {
                    ...
                    if (reg == 2) ILOP("LGDT", 0x0F, 0x01, opc);
                    if (reg == 3) ILOP("LIDT", 0x0F, 0x01, opc);
                    if (reg == 7) ILOP("INVLPG", 0x0F, 0x01, opc);
                }
                if (reg == 6 || reg == 7)
                    UNKN(0x0F, 0x00, opc);
                if (reg == 5)
                    UNKN(0x0F, 0x01, opc);  // reg=7 is INVLPG in i80486
                if (reg < 4)
                    UNKN(0x0F, 0xBA, opc);
            }
        }
...
void run_tests(const char *cpu) {
    ...
    if (is80386())
        RUN_TEST(test_illegal_80386);
    if (is80486())
        RUN_TEST(test_illegal_80486);
}
