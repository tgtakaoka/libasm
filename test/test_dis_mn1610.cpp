...
static void test_misc() {
    ...
    if (is1613()) {
        ...
        ERRR(LB, "CSBR, 0x1234", 0x0F07|(0<<4), 0x1234);
        ...
        ERRR(SETB, "STR, CSBR", 0x0F00|(0<<4)|6);
        ...
        ERIR(LS, "SBRB, 0x1234", 0x0F0F|(3<<4), 0x1234);
        ERIR(LS, "SBRB, 0x1234", 0x0F0F|(4<<4), 0x1234);
        ERIR(LS, "SBRB, 0x1234", 0x0F0F|(5<<4), 0x1234);
        ERIR(LS, "SBRB, 0x1234", 0x0F0F|(6<<4), 0x1234);
        ERIR(LS, "SBRB, 0x1234", 0x0F0F|(7<<4), 0x1234);
        TEST(STS, "SBRB, 0x1234", 0x0F8F|(0<<4), 0x1234);
        TEST(STS, "ICB, 0x1234",  0x0F8F|(1<<4), 0x1234);
        TEST(STS, "NPP, 0x1234",  0x0F8F|(2<<4), 0x1234);
        ERIR(STS, "SBRB, 0x1234", 0x0F8F|(3<<4), 0x1234);
        ERIR(STS, "SBRB, 0x1234", 0x0F8F|(4<<4), 0x1234);
        ERIR(STS, "SBRB, 0x1234", 0x0F8F|(5<<4), 0x1234);
        ERIR(STS, "SBRB, 0x1234", 0x0F8F|(6<<4), 0x1234);
        ERIR(STS, "SBRB, 0x1234", 0x0F8F|(7<<4), 0x1234);
        TEST(CPYS, "R2, SBRB", 0x0F88|(0<<4)|2);
        TEST(CPYS, "R1, ICB",  0x0F88|(1<<4)|1);
        TEST(CPYS, "R0, NPP",  0x0F88|(2<<4)|0);
        ERIR(CPYS, "R3, SBRB", 0x0F88|(3<<4)|3);
        ERIR(CPYS, "R4, SBRB", 0x0F88|(4<<4)|4);
        ERIR(CPYS, "SP, SBRB", 0x0F88|(5<<4)|6);
        ERIR(CPYS, "R1, SBRB", 0x0F88|(6<<4)|1);
        ERIR(CPYS, "R2, SBRB", 0x0F88|(7<<4)|2);
        TEST(SETS, "R2, SBRB", 0x0F08|(0<<4)|2);
        TEST(SETS, "R1, ICB",  0x0F08|(1<<4)|1);
        TEST(SETS, "R0, NPP",  0x0F08|(2<<4)|0);
        ERIR(SETS, "R3, SBRB", 0x0F08|(3<<4)|3);
        ERIR(SETS, "R4, SBRB", 0x0F08|(4<<4)|4);
        ERIR(SETS, "SP, SBRB", 0x0F08|(5<<4)|6);
        ERIR(SETS, "R1, SBRB", 0x0F08|(6<<4)|1);
        ERIR(SETS, "R2, SBRB", 0x0F08|(7<<4)|2);
...
#define ERROR_T(err, ...) assert_error(__FILE__, __LINE__, err, __VA_ARGS__)
#define UNKNOWN(...) ERROR_T(UNKNOWN_INSTRUCTION, __VA_ARGS__)
...
static void print_illegal(Config::opcode_t start = 0, Config::opcode_t end = 0xffff,
    ...
    bool prev = true;
    auto fails = 0;
    Config::opcode_t fail_start = start;
    bool quit = false;
    for (Config::opcode_t hi = 0; hi < 0x20 && !quit; hi++) {
        for (Config::opcode_t lo = offset; lo < 0x800 && !quit; lo += step) {
            const Config::opcode_t opc = (hi << 11) | lo;
            if (opc < start)
                continue;
            if (opc >= end)
                quit = true;
    ...
    printf("%04X-%04X offset=%d step=%d errors %d\n", start, end, offset, step, errors);
    errors = 0;
}
...
static void test_illegal_mn1613() {
    ...
    for (auto opc = 0x0800; opc < 0x1000; opc++) {
        const auto rd = (opc >> 8) & 7;
        if (rd != 7)
            continue;  // MVI
        if (opc & 0x08) {
            ...
            if (rb == 0 && (opc & 0x80) == 0)
                ERROR_T(REGISTER_NOT_ALLOWED, opc);
        }
    }

    for (auto opc = 0x1000; opc < 0x2000; opc++) {
        const auto rd = (opc >> 8) & 7;
        const auto sk = (opc >> 4) & 0xF;
        const auto rs = opc & 7;
    ...
    for (auto opc = 0x2000; opc < 0x2800; opc++) {
        ...
        case 0:  // H, WTR, LR
            if (rd >= 6) {
                ...
                ERROR_T(ILLEGAL_OPERAND_MODE, opc);
            } else if (sk == 2 || sk == 3) {
                ERROR_T(ILLEGAL_OPERAND_MODE, opc);
            }
            break;
    ...
    for (auto opc = 0x4000; opc < 0x5000; opc++) {
        const auto rd = (opc >> 8) & 7;
        if (rd == 7 && (opc & 4) == 0)
            ERROR_T(ILLEGAL_REGISTER, opc);
    }

    for (auto opc = 0x7000; opc < 0x7800; opc++) {
        const auto rd = (opc >> 8) & 7;
        const auto rs = opc & 7;
        if (rd < 7 && rs == 7) {
            ERROR_T(ILLEGAL_REGISTER, opc);
    ...
    for (auto opc = 0x7800; opc < 0x8000; opc++) {
        const auto rd = (opc >> 8) & 7;
        const auto rs = opc & 7;
        if (rd < 7 && rs == 7 && (opc & 8) == 0) {
            ERROR_T(ILLEGAL_REGISTER, opc);
