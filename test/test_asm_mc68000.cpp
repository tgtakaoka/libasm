...
static void test_float_move() {
    ...
    TEST("FMOVE.P #9.12E20, FP1",       0xF200|074, 0x4000|(3<<10)|(1<<7),
         0x0020, 0x0009, 0x1200, 0x0000, 0x0000, 0x0004);
...
static void test_float_arithmetic() {
    ...
    TEST("FTST.X FP0, FP1",            0xF200,     0x003A|(0<<10)|(1<<7));
    TEST("FTST.L D2, FP3",             0xF200|002, 0x403A|(0<<10)|(3<<7));
    ERRT("FTST.S A4, FP5",             OPERAND_NOT_ALLOWED, "A4, FP5");
    TEST("FTST.X (A6), FP7",           0xF200|026, 0x403A|(2<<10)|(7<<7));
    TEST("FTST.P (A0)+, FP1",          0xF200|030, 0x403A|(3<<10)|(1<<7));
    TEST("FTST.W -(A2), FP3",          0xF200|042, 0x403A|(4<<10)|(3<<7));
    TEST("FTST.D ($1234,A4), FP5",     0xF200|054, 0x403A|(5<<10)|(5<<7), 0x1234);
    TEST("FTST.B ($23,A6,D7.W), FP0",  0xF200|066, 0x403A|(6<<10)|(0<<7), 0x7023);
    TEST("FTST.L ($004566).W, FP1",    0xF200|070, 0x403A|(0<<10)|(1<<7), 0x4566);
    TEST("FTST.S ($56789A).L, FP2",    0xF200|071, 0x403A|(1<<10)|(2<<7), 0x0056, 0x789A);
    TEST("FTST.X (*+$1234,PC), FP3",   0xF200|072, 0x403A|(2<<10)|(3<<7), 0x1230);
    TEST("FTST.P (*+90,PC,A4.L), FP5", 0xF200|073, 0x403A|(3<<10)|(5<<7), 0xC856);
    TEST("FTST.L #$6789ABCD, FP6",     0xF200|074, 0x403A|(0<<10)|(6<<7), 0x6789, 0xABCD);
    TEST("FTST.S #7.88999976E-10, FP7",0xF200|074, 0x403A|(1<<10)|(7<<7), 0x3058, 0xE0F0);
    TEST("FTST.X #-89000000032, FP0",  0xF200|074, 0x403A|(2<<10)|(0<<7),
         0xC023, 0x0000, 0xA5C6, 0x81D1, 0x0000, 0x0000);
    TEST("FTST.P #9.12E20, FP1",       0xF200|074, 0x403A|(3<<10)|(1<<7),
         0x0020, 0x0009, 0x1200, 0x0000, 0x0000, 0x0004);
    TEST("FTST.W #$1234, FP2",         0xF200|074, 0x403A|(4<<10)|(2<<7), 0x1234);
    TEST("FTST.D #-8.25, FP3",         0xF200|074, 0x403A|(5<<10)|(3<<7),
         0xC020, 0x8000, 0x0000, 0x0000);
    TEST("FTST.B #$23, FP4",           0xF200|074, 0x403A|(6<<10)|(4<<7), 0x0023);
}
...
static void test_data_constant() {
    ...
    TEST("DC.S  1.0, -inf, +nan, -2.25e2",
         ...
         0x7FC0, 0x0000,
         0xC361, 0x0000);
    TEST("DC.D -1.0, +inf, -nan, -2.25e2",
         0xBFF0, 0x0000, 0x0000, 0x0000,
         0x7FF0, 0x0000, 0x0000, 0x0000,
         0xFFF8, 0x0000, 0x0000, 0x0000,
         0xC06C, 0x2000, 0x0000, 0x0000);
    TEST("DC.X -2.25, +inf, -nan, -9.87e123",
         ...
         0xC19A, 0x0000, 0xEEE3, 0xF8B5, 0x7D26, 0xD000);
    TEST("DC.P -2.25, +inf, -nan, -22.5, -0.225",
