...
void test_sdbd() {
    ...
    DTEST(0x0100, "SDBD", 0x0001,
          0x0101, 0x0101, "MVII X'1234', R0", 0x02B8, 0x0034, 0x0012);
    DTEST(0x0100, "SDBD", 0x0001,
          0x0101, 0x0101, "ADDI X'ABCD', R1", 0x02F9, 0x00CD, 0x00AB);
    DTEST(0x0100, "SDBD", 0x0001,
          0x0101, 0x0101, "SUBI X'5678', R2", 0x033A, 0x0078, 0x0056);
    DTEST(0x0100, "SDBD", 0x0001,
          0x0101, 0x0101, "CMPI X'DEF0', R3", 0x037B, 0x00F0, 0x00DE);
    DTEST(0x0100, "SDBD", 0x0001,
          0x0101, 0x0101, "ANDI X'1234', R4", 0x03BC, 0x0034, 0x0012);
    DTEST(0x0100, "SDBD", 0x0001,
          0x0101, 0x0101, "XORI X'FFFF', R5", 0x03FD, 0x00FF, 0x00FF);
    // Indirect: instruction encoding unchanged; CPU reads two words via pointer.
    DTEST(0x0100, "SDBD", 0x0001,
          0x0101, 0x0101, "MVI@ R1, R0", 0x0288);
    DTEST(0x0100, "SDBD", 0x0001,
          0x0101, 0x0101, "ADD@ R4, R2", 0x02E2);
    DTEST(0x0100, "SDBD", 0x0001,
          0x0101, 0x0101, "XOR@ R5, R7", 0x03EF);
    // No-effect: SDBD before an unsupported instruction is flagged.
    DERRT(0x0100, "SDBD", 0x0001,
          0x0101, 0x0101, "EIS",             0x0002);
    DERRT(0x0100, "SDBD", 0x0001,
          0x0101, 0x0101, "MVO R0, X'1234'", 0x0240, 0x1234);
    DERRT(0x0100, "SDBD", 0x0001,
          0x0101, 0x0101, "MVO@ R0, R1",     0x0248);
    DERRT(0x0100, "SDBD", 0x0001,
          0x0101, 0x0101, "MVOI R0, X'34'",  0x0278, 0x0034);
    DERRT(0x0100, "SDBD", 0x0001,
          0x0101, 0x0101, "MVI X'1234', R0", 0x0280, 0x1234);
    DERRT(0x0100, "SDBD", 0x0001,
          0x0101, 0x0101, "PULR R0",         0x02B0);
}
