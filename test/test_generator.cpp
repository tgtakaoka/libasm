...
TestGenerator::TestGenerator(Formatter &formatter, Disassembler &disassembler, uint32_t addr)
    : _formatter(formatter),
      ...
      _hasLongInstruction(false) {}

TestGenerator::~TestGenerator() {
    delete[] _memory;
}

TestGenerator &TestGenerator::hasLongInstruction() {
    _hasLongInstruction = true;
    return *this;
}
...
const TokenizedText &TestGenerator::meaningfulTestData(std::string &name) {
    name += _listing.name();
    const auto size = _listing.length();
    if (!_hasLongInstruction || size < 8) {
        name += ':';
        name += size + '0';
    }
