...
// |Hex| = IntelNumber (native ..H) or CstyleNumber (gnu-as 0x..).  Consolidate
// register-indexed displacement sign "[reg+n]"/"[reg-n]" -> "[reg+n]".
template <class Hex>
TokenizerList tokenizers(char loc) {
    static const RegDispTokenizer<Hex, BracketReg> regDisp;
    TokenizerList list = standardTokenizers<Hex>(loc, {&regDisp});
    list.push_back(&REG8);
