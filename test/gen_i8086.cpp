...
// The number families fold a leading '-' into their single 'n' token, but an
// 80X86 positive displacement is printed as "+n"; drop the leading '+' so a
// number tokenizer reduces the digits the same way. This collapses e.g.
// [eax+ecx*4+n] and [eax+ecx*4-n] (base+index*scale displacements, which the
// [REG+n] rules do not reach) to one token. The '+' between a base and an
// index register (before a letter) is left intact.
struct X86PlusTokenizer : Tokenizer {
    const char *tokenize(const char *p, std::string &out) const override {
        if (p[0] == '+' && isdigit(p[1]))
            return p + 1;
        return nullptr;
