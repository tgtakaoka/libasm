...
bool mb8861() {
    return strcasecmp_P("MB8861", assembler.config().cpu_P()) == 0;
}

bool m6800() {
    return strcmp_P("6800", assembler.config().cpu_P()) == 0 || mb8861();
}

bool hd6301() {
    return strcmp_P("6301", assembler.config().cpu_P()) == 0;
}

bool m68hc11() {
    return strcmp_P("6811", assembler.config().cpu_P()) == 0;
}

bool m6801() {
    return strcmp_P("6801", assembler.config().cpu_P()) == 0 || hd6301() || m68hc11();
}
...
void test_cpu() {
    ...
    EQUALS_P("cpu 6800", "6800", assembler.config().cpu_P());

    EQUALS("cpu 6801", true,   assembler.setCpu("6801"));
    EQUALS_P("cpu 6801", "6801", assembler.config().cpu_P());

    EQUALS("cpu 6301", true,   assembler.setCpu("6301"));
    EQUALS_P("cpu 6301", "6301", assembler.config().cpu_P());

    EQUALS("cpu 6811", true,   assembler.setCpu("6811"));
    EQUALS_P("cpu 6811", "6811", assembler.config().cpu_P());

    EQUALS("cpu MC6800", true,   assembler.setCpu("mc6800"));
    EQUALS_P("cpu MC6800", "6800", assembler.config().cpu_P());

    EQUALS("cpu mb8861", true, assembler.setCpu("mb8861"));
    EQUALS_P("cpu MB8861", "MB8861", assembler.config().cpu_P());

    EQUALS("cpu MC6801", true,   assembler.setCpu("mc6801"));
    EQUALS_P("cpu MC6801", "6801", assembler.config().cpu_P());

    EQUALS("cpu HD6301", true,   assembler.setCpu("hd6301"));
    EQUALS_P("cpu HD6301", "6301", assembler.config().cpu_P());

    EQUALS("cpu MC6811", true,   assembler.setCpu("mc6811"));
    EQUALS_P("cpu MC6811", "6811", assembler.config().cpu_P());

    EQUALS("cpu MC68HC11", true,   assembler.setCpu("mc68hc11"));
    EQUALS_P("cpu MC68HC11", "6811", assembler.config().cpu_P());
}
