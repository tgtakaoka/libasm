...
void IntelHex::reset(AddressWidth width, AddressUnit unit, uint8_t recordSize) {
    BinEncoder::reset(width, unit, recordSize);
    reset();
}
...
void IntelHex::encode(TextPrinter &out, uint32_t addr, const uint8_t *data, uint_fast8_t size) {
    ...
    if ((addr & ~UINT16_MAX) != 0 && ((addr ^ _last_addr) >> 16) != 0) {
        formatEla(out, addr);
    }
...
void IntelHex::formatEla(TextPrinter &out, uint32_t addr) {
    if (addressSize(_addr_width, _addr_unit) <= 2)
        return;
