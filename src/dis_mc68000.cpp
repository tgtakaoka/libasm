...
void DisMc68000::decodeBriefExtension(
    ...
    if (indexScale && !firstGen()) {
        auto scale = 1 << indexScale;
        out.letter('*').dec(scale);
    }
}
...
void DisMc68000::decodeFullExtension(
    ...
    if ((ext & 7) == 0) {
        ...
        out.letter('(');
        auto delim = outDisplacement(insn, out, base, disp, bdSize, PSTR_NULL);
        if ((ext & 0x80) == 0) {
...
#if !defined(LIBASM_MC68000_NOFPU)
    case M_KDREG:
        outRegName(out.letter('{'), decodeDataReg((insn.postfix() >> 4) & 7)).letter('}');
        break;
    case M_FPMLT:
...
#if !defined(LIBASM_MC68000_NOMMU)
    case M_IMFC: {
        const auto fc = insn.postfix() & 0xF;
        if (mc68030() && fc >= 8)
            insn.setErrorIf(out, OVERFLOW_RANGE);
        outDec(out.letter('#'), fc, 4);
        break;
    }
    case M_IMFM: {
        const auto fm = (insn.postfix() >> 5) & 0xF;
        if (mc68030() && fm >= 8)
            insn.setErrorIf(out, OVERFLOW_RANGE);
        outDec(out.letter('#'), fm & 0xF, 4);
        break;
    }
    case M_IMLV:
...
char DisMc68000::decodeInsnSize(const DisInsn &insn, OprSize osize) const {
