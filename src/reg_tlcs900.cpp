...
Size ctrlRegSize(RegName name) {
    if (name == REG_XNSP) return SZ_QUAD;
    if (name == REG_NSP)  return SZ_WORD;
    if (name == REG_INTNEST) return SZ_WORD;
    const uint8_t sub = ctrlRegCode(name);
    if (sub <= 0x1C) return SZ_QUAD;                  // DMAS0-3, DMAD0-3: 32-bit
...
RegName decodeCtrlReg(uint8_t sub, bool is32, bool isBase) {
    if (sub == 0x3C) {
        if (is32) return isBase ? REG_XNSP : REG_UNDEF;
        return isBase ? REG_NSP : REG_INTNEST;
    }
    if (sub & 1) return REG_UNDEF;              // odd sub-bytes invalid
    if (sub <= 0x1C) return (sub & 3) ? REG_UNDEF : RegName(sub + uint8_t(REG_DMAS0));
    if (sub <= 0x2E) return RegName(sub + uint8_t(REG_DMAS0));
    return REG_UNDEF;
}
...
RegName parseCtrlRegName(StrScanner &scan, const ValueParser &parser) {
    ...
    if (token.iequals_P(TEXT_REG_NSP))     { scan = p; return REG_NSP;  }
    if (token.iequals_P(TEXT_REG_XNSP))    { scan = p; return REG_XNSP; }
    if (token.iequals_P(TEXT_REG_INTNEST)) { scan = p; return REG_INTNEST; }
    // DMA[SDCM][0-3]: token must be exactly 5 chars "DMAxy"
    if (token.size() != 5) return REG_UNDEF;
    const char *t = token.str();
    if ((t[0] | 0x20) != 'd' || (t[1] | 0x20) != 'm' || (t[2] | 0x20) != 'a')
        return REG_UNDEF;
    if (t[4] < '0' || t[4] > '3') return REG_UNDEF;
    const uint8_t idx = uint8_t(t[4] - '0');
    uint8_t sub;
    switch (t[3] | 0x20) {
    case 's': sub = uint8_t(idx * 4);        break;  // DMAS: 0x00..0x0C
    case 'd': sub = uint8_t(0x10 + idx * 4); break;  // DMAD: 0x10..0x1C
    case 'c': sub = uint8_t(0x20 + idx * 4); break;  // DMAC: 0x20..0x2C
    case 'm': sub = uint8_t(0x22 + idx * 4); break;  // DMAM: 0x22..0x2E
...
StrBuffer &outCtrlRegName(StrBuffer &out, RegName name) {
    if (name == REG_XNSP)    return out.text_P(TEXT_REG_XNSP);
    if (name == REG_NSP)     return out.text_P(TEXT_REG_NSP);
    if (name == REG_INTNEST) return out.text_P(TEXT_REG_INTNEST);
    const uint8_t sub = ctrlRegCode(name);
    const uint8_t idx = (sub >> 2) & 3;
    char type;
    if (sub < 0x10) type = 'S';
    else if (sub < 0x20) type = 'D';
    else type = (sub & 2) ? 'M' : 'C';
...
static const NameTable *abregTable(uint8_t size, bool fixed) {
    switch (size) {
    case 0: return fixed ? &ABREG8_FIXED_TABLE  : &ABREG8_REL_TABLE;
    case 1: return fixed ? &ABREG16_FIXED_TABLE : &ABREG16_REL_TABLE;
    case 2: return fixed ? nullptr              : &ABREG32_REL_TABLE;
...
bool parseAbsRegCode(StrScanner &scan, const ValueParser &parser, uint8_t &code, uint8_t &size) {
    ...
    const StrScanner base{token.str(), hasBankDigit ? token.str() + token.size() - 1
                                                    : token.str() + token.size()};
