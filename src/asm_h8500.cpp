...
PROGMEM constexpr AsmH8500::PseudosH8500 AsmH8500::PSEUDOS_H8500{
        ARRAY_RANGE(PSEUDO_H8500_TABLE)};
...
Error AsmH8500::parseOperand(StrScanner &scan, Operand &op) const {
    ...
    if (p.expect('@')) {
        ...
        if (p.expect('(')) {
            ...
            if (!p.skipSpaces().expect(',')) return op.setError(p, MISSING_COMMA);
            const auto at = p.skipSpaces();
            const auto reg = parseRegName(p, parser());
            if (reg == REG_UNDEF) return op.setError(at, UNKNOWN_REGISTER);
            if (!p.skipSpaces().expect(')')) return op.setError(p, MISSING_CLOSING_PAREN);
            op.val = disp;
            op.reg = reg;
    ...
    if (p.expect('{')) {
        ...
        if (!p.skipSpaces().expect('}')) return op.setError(p, MISSING_CLOSING_BRACKET);
        op.mode = M_REGLIST;
