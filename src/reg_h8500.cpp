...
constexpr NameEntry CR_ENTRIES[] PROGMEM = {
    { TEXT_REG_BR,  CR_BR   },
    { TEXT_REG_CCR, CR_CCR  },
    // CP (code page) is recognised so that "LDC R0, CP" / "STC CP, R0" can
    // surface a clear "register not allowed" rather than the generic
    // "unknown operand". CP exists in the H8/500 register file but is not
    // addressable via LDC/STC, so it maps to the CR_UNDEF sentinel (-1).
    { TEXT_REG_CP,  -1      },
    { TEXT_REG_DP,  CR_DP   },
    { TEXT_REG_EP,  CR_EP   },
    { TEXT_REG_SR,  CR_SR   },
