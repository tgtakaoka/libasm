...
constexpr Entry MC68030_TABLE[] PROGMEM = {
...
constexpr uint8_t MC68030_INDEX[] PROGMEM = {
      1,  // TEXT_PFLUSH
      2,  // TEXT_PFLUSH
      3,  // TEXT_PFLUSH
      4,  // TEXT_PFLUSH
...
constexpr EntryPage MC68030_PAGES[] PROGMEM = {
        ...
        {ARRAY_RANGE(MC68010_TABLE), ARRAY_RANGE(MC68010_INDEX)},
        {ARRAY_RANGE(MC68000_TABLE), ARRAY_RANGE(MC68000_INDEX)},
        {ARRAY_RANGE(MC68030_TABLE), ARRAY_RANGE(MC68030_INDEX)},
};
...
const Mmu *mmu(CpuSpec cpuSpec) {
    const auto mmuType = (cpuSpec.cpu == MC68020) ? cpuSpec.mmu : MMU_NONE;
...
#if !defined(LIBASM_MC68000_NOMMU)
    if (!insn.isOK())
        mmu(cpuSpec)->searchName(insn, Cpu::acceptAll<AsmInsn, Entry>);
#endif
...
bool acceptSize(const AsmInsn &insn, const Entry::Flags &flags) {
    ...
    if (insnSize == ISZ_BYTE)
        return opr == SZ_DATA
               ...
               || opr == SZ_FDAT
#endif
               || opr == SZ_DATH || opr == SZ_CAS1 || opr == SZ_PMMU;
    if (insnSize == ISZ_WORD || insnSize == ISZ_LONG)
        return opr == SZ_DATA || opr == SZ_ADDR || opr == SZ_ADR8
#if !defined(LIBASM_MC68000_NOFPU)
               || opr == SZ_FDAT
#endif
               || opr == SZ_DATH || opr == SZ_CAS1 || opr == SZ_CAS2 || opr == SZ_PMMU;
...
#if !defined(LIBASM_MC68000_NOMMU)
    if (insn.getError() == UNKNOWN_INSTRUCTION) {
        mmu(cpuSpec)->searchName(insn, acceptModes);
    }
#endif
...
#if !defined(LIBASM_MC68000_NOMMU)
    if (insn.getError() == UNKNOWN_INSTRUCTION)
        mmu(cpuSpec)->searchOpCode(insn, out, matchOpCode);
#endif
    if (insn.getError() == UNKNOWN_INSTRUCTION)
