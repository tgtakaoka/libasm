...
void AsmZ380::encodeShortIndex(AsmInsn &insn, const Operand &op, AddrMode mode) const {
    if (z380()) {
        auto fixup = DD_UNDEF;
        if (!_ddir) {
        do_fixup:
...
Error AsmZ380::parseOperand(StrScanner &scan, Operand &op, const AsmInsn &insn) const {
