...
enum AddrMode : uint8_t {
    ...
    M_RMEM = 19,   // Readable Memory:         (An)/(An)+/-(An)/(*,An)/(Abs)/(*,PC)
    M_WMEM = 20,   // Writable Memory:         (An)/(An)+/-(An)/(*,An)/(Abs)
    M_JADDR = 21,  // Jumpable Address:        (An)            /(*,An)/(Abs)/(*,PC)
    M_IADDR = 22,  // Increment Address:       (An)/(An)+      /(*,An)/(Abs)/(*,PC)
    M_DADDR = 23,  // Decrement Address:       (An)/     /-(An)/(*,An)/(Abs)
    M_BITFR = 24,  // Bitfield read:     Dn/  /(An)/           /(*,An)/(Abs)/(*,PC)
    M_BITFW = 25,  // Bitfield write:    Dn/  /(An)/           /(*,An)/(Abs)
