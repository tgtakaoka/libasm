...
enum AddrMode : uint8_t {
    ...
    M_INDX = 7,    // (d8,An,Xn): Address Register Indirect with Index
    ...
    M_PCIDX = 13,  // (d8,PC,Xn): Program Counter Indirect with Index
    // for instruction table; M_RADDR-M_DADDR must be contigous.
    M_RADDR = 18,  // Readable Address:  Dn/An/(An)/(An)+/-(An)/(*,An)/(Abs)/(*,PC)/#xxxx
    M_RDATA = 19,  // Readable Data:     Dn   /(An)/(An)+/-(An)/(*,An)/(Abs)/(*,PC)/#xxxx
    M_WADDR = 20,  // Readable Address:  Dn/An/(An)/(An)+/-(An)/(*,An)/(Abs)
    M_WDATA = 21,  // Writable Data:     Dn   /(An)/(An)+/-(An)/(*.An)/(Abs)
    M_RMEM = 22,   // Readable Memory:         (An)/(An)+/-(An)/(*,An)/(Abs)/(*,PC)
    M_WMEM = 23,   // Writable Memory:         (An)/(An)+/-(An)/(*,An)/(Abs)
    M_JADDR = 24,  // Jumpable Address:        (An)            /(*,An)/(Abs)/(*,PC)
    M_IADDR = 25,  // Increment Address:       (An)/(An)+      /(*,An)/(Abs)/(*,PC)
    M_DADDR = 26,  // Decrement Address:       (An)/     /-(An)/(*,An)/(Abs)
    // for instruction table
    M_REL8 = 27,   // 8/16-bit Relative (Bcc)
    M_REL16 = 28,  // 16-bit Relative (DBcc/FDBcc)
    M_IMBIT = 29,  // Bit number: #0~#7/#15/#31
    M_IM3 = 30,    // 3-bit Immediate: #1~#8
    M_IM8 = 31,    // 8-bit Immediate
    M_IMVEC = 32,  // 3-bit Trap Vector
    M_IMDSP = 33,  // 16-bit Signed Displacement
    M_LABEL = 34,  // label
    M_MULT = 35,   // MOVEM register list
    M_SR = 36,     // SR register
    M_CCR = 37,    // CCR register
    M_USP = 38,    // USP register
    // MC68010
    M_CREG = 39,  // Control register: SFC, DFC, USP, VBR
    // MC68881
    M_FPREG = 40,  // FPn: Floatng point data register
    M_FPMLT = 41,  // FMOVEM.X register list; FPn
    M_FCMLT = 42,  // FMOVEM.L register list; FPCR/FPSR/FPIAR
    M_FSICO = 43,  // FPSINCOS FPc:FPs
    M_KDREG = 44,  // <ea>{Dn}
    M_KFACT = 45,  // <ea>{#k}
    M_FPCR = 46,   // FPCR register
    M_FPSR = 47,   // FPSR register
    M_FPIAR = 48,  // FPIAR register
    M_IMROM = 49,  // MC68881 ROM constant
    M_REL32 = 50,  // 32-bit Relative; 1111|ccc|01s|___|___: s=0 16bit, s=1 32bit (FBcc)
};
