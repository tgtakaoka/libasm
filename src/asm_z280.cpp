...
void AsmZ280::encodeAbsolute(AsmInsn &insn, const Operand &op, AddrMode mode) const {
...
void AsmZ280::encodeShortIndex(AsmInsn &insn, const Operand &op, AddrMode mode) const {
    if (op.val.overflowInt8())
        insn.setErrorIf(op, OVERFLOW_RANGE);
    insn.emitOperand8(op.val.getUnsigned());
}

void AsmZ280::encodeImmediate16(AsmInsn &insn, const Operand &op, AddrMode mode) const {
    if (op.val.overflowUint16())
        insn.setErrorIf(op, OVERFLOW_RANGE);
    insn.emitOperand16(op.val.getUnsigned());
}
...
Error AsmZ280::parseOperand(StrScanner &scan, Operand &op, const AsmInsn &insn) const {
