...
void AsmInsn::applyAutoSizePrefix() {
    ...
    if (!model32()) {
        ...
        // The MOVSX/MOVZX word source (M_WMOD16) is intrinsically 16-bit but
        // never drives DATA32 here since the source-based terms below only
        // test M_WMOD/M_WMEM, not M_WMOD16.
        const auto dstIsSreg = dst() == M_SREG;
        const auto isFpuEscape = AsmInsn::escapeInsn(prefix());
        if (!dstIsSreg && !noData32() && !isFpuEscape &&
                ((is16BitReg(dstMode) && (dst() == M_WREG || dst() == M_WMOD)) ||
                        (is16BitReg(srcMode) && (src() == M_WREG || src() == M_WMOD)) ||
                        (size() == SZ_DATA &&
                                (is16BitReg(dstMode) ||
                                        (is16BitReg(srcMode) && src() != M_WMOD16) ||
                                        is16BitReg(extMode))) ||
