...
int8_t postPos(OprPos pos) {
    static constexpr int8_t BITNO[] PROGMEM = {
            ...
            0,   // EX_PFC = 22: _____________|nnnn : function code
            5,   // EX_PFM = 23: ______|mmmm|______ : function code mask
            2,   // EX_PR = 24:__|rrrr|___|nnn|__ : PMMU register
            5,   // EX_PAR = 25:  _________aaa|_____ : PTEST An
    };
    if (pos < EX_RX)
        return -1;
...
bool AsmMc68000::encodeMiscImmediate(
...
void AsmMc68000::encodeImmediate(AsmInsn &insn, const Operand &op, OprSize size) const {
    ...
    case SZ_BYTE:
        ...
        break;
    case SZ_QUAD:
        insn.emitOperand64(op.val.getInteger());
        break;
    ...
    case SZ_PBCD:
        ...
        insn.emitUint64(0, insn.operandPos());
        break;
#elif !defined(LIBASM_MC68000_NOFPU)
    case SZ_SNGL:
...
void AsmMc68000::encodePmove(AsmInsn &insn, const Operand &op, AddrMode mode) const {
    ...
    if (regSize == SZ_QUAD) {
        const auto &other = (&op == &insn.srcOp) ? insn.dstOp : insn.srcOp;
...
bool validDataReg(AsmInsn &insn, AddrMode mode, const ErrorAt &at) {
    if (mode >= M_RMEM) {
        insn.reset();
...
bool validAddrReg(AsmInsn &insn, AddrMode mode, const ErrorAt &at) {
...
bool validImmediate(AsmInsn &insn, AddrMode mode, const ErrorAt &at) {
    if (mode >= M_WADDR) {
#if !defined(LIBASM_MC68000_NOMMU)
        if (mode == M_MROOT)
            return true;
...
Error AsmMc68000::encodeOperand(
    ...
    case M_DREG:
        ...
        break;
    case M_AREG:
        if (validAddrReg(insn, mode, op)) {
            const auto post_gp = encodeModeReg(insn, op.mode, op.reg, pos);
            if (size == SZ_BYTE) {
                if (post_gp < 0 && insn.oprSize() != SZ_PMMU) {
    ...
    case M_AIND:
        ...
        break;
    case M_PINC:
        if (validPostInc(insn, mode, op))
            encodeModeReg(insn, op.mode, op.addr.base, pos);
        break;
    case M_PDEC:
        if (validPreDec(insn, mode, op))
            encodeModeReg(insn, op.mode, op.addr.base, pos);
        break;
    case M_PCDSP:
        if (invalidPcBase(insn, mode, op))
            break;
    ...
    case M_ALONG:
        ...
        break;
    case M_IMDAT:
        if (encodeMiscImmediate(insn, op, mode, pos, size))
            break;
        /* Fall-through */
#if !defined(LIBASM_MC68000_NOFPU)
    case M_IMFLT:
#endif
        if (validImmediate(insn, mode, op)) {
