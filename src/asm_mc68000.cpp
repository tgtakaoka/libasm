...
void emitPackedBcd96(AsmInsn &insn, const Operand &op) {
    ...
    if (isinf(v)) {
        ...
        int16_t exponent = log10l(v);
        if (exponent < 0) {
            word |= 0x4000;
            exponent = -exponent;
        }
        v /= powl(10.0, exponent);
        insn.emitOperand16(word | convert2pbcd(exponent, 3));
        insn.emitOperand16(v);
        insn.emitOperand64(convert2pbcd(v * powl(10.0, 16), 16));
    }
}
