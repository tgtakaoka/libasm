...
inline uint8_t fieldN(uint16_t w) { return (w >> 8) & 0xF; }
inline uint8_t fieldM(uint16_t w) { return (w >> 4) & 0xF; }
...
void DisSuperH::decodeOperand(DisInsn &insn, StrBuffer &out, AddrMode mode) const {
    ...
    case M_PCL: {
        ...
        const auto target = (mode == M_PCW)
                ? pc4 + d * 2u
                : (pc4 & ~Config::uintptr_t(3)) + d * 4u;
        out.text_P(PSTR("@("));
    ...
    case M_REL8: {
        ...
        const auto target = static_cast<Config::uintptr_t>(
                insn.address() + 4 + static_cast<int32_t>(d) * 2);
        outRelAddr(out, target, insn.address(), 9);
        break;
    }
    case M_REL12: {
        auto d = static_cast<int16_t>(opc & 0xFFF);
        if (d & 0x800) d |= 0xF000;  // sign-extend 12->16
        const auto target = static_cast<Config::uintptr_t>(
                insn.address() + 4 + static_cast<int32_t>(d) * 2);
        outRelAddr(out, target, insn.address(), 13);
        break;
    }
    case M_SR:   outRegName(out, REG_SR);   break;
    case M_GBR:  outRegName(out, REG_GBR);  break;
    case M_VBR:  outRegName(out, REG_VBR);  break;
    case M_MACH: outRegName(out, REG_MACH); break;
    case M_MACL: outRegName(out, REG_MACL); break;
    case M_PR:   outRegName(out, REG_PR);   break;
    case M_IGBR:
        out.text_P(PSTR("@(R0,"));
        outRegName(out, REG_GBR);
        out.letter(')');
