...
struct Entry final : entry::Base<Config::opcode_t> {
    struct Flags final {
        ...
        static constexpr Flags create(AddrMode dst, AddrMode src) {
            return Flags{static_cast<uint16_t>((dst << dst_gp) | (src << src_gp))};
        }
    ...
    private:
        ...
        static constexpr int src_gp = 7;
        static constexpr uint_fast8_t dst_gm = 0x7F;
        static constexpr uint_fast8_t src_gm = 0x7F;
    };

    constexpr Entry(Config::opcode_t opc, Config::opcode_t mask, Flags flags,
            const /* PROGMEM */ char *name_P)
        : Base(name_P, opc), _flags_P(flags), _mask_P(mask) {}

    Flags readFlags() const { return Flags{pgm_read_word(&_flags_P._attr)}; }
    uint8_t readMask() const { return pgm_read_byte(&_mask_P); }

private:
    const Flags _flags_P;
    const uint8_t _mask_P;
};
