...
Error AsmMc6809::encodeStackOp(InsnMc6809 &insn) {
    uint8_t post = 0;
    const char *p = _scan;
    while (*p) {
        host::uint_t bit = 0;
        ...
        for (host::uint_t index = 0, mask = 0x01; index < 8; index++, mask <<= 1) {
            const RegName regName = _regs.getStackReg(index, insn.opCode());
...
Error AsmMc6809::processPseudo(InsnMc6809 &insn) {
    ...
    if (insn.opCode() == PSEUDO_SETDP)
        return setError(getOperand(_direct_page));
