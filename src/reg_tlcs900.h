...
enum RegName : int8_t {
    ...
    REG_SR = 24,
    // TLCS-900
    REG_XNSP = 32,   // XNSP: 32-bit Normal Stack Pointer (ctrl sub-byte 0x3C; base only)
    REG_NSP = 33,    // NSP:  16-bit Normal Stack Pointer (ctrl sub-byte 0x3C; base only)
    REG_INTNEST = 34,// INTNEST: 16-bit Interrupt Nesting Counter (ctrl sub-byte 0x3C; non-base only)
    // DMA control registers (accessible only via LDC).
    // RegName value = ctrl sub-byte + 0x40; ctrlRegCode(r) = uint8_t(r) - 0x40.
...
// Parse an absolute bank register name from |scan|.
// On success, sets |code| to the register code byte and |size| to 0/1/2
// for 8/16/32-bit, advances |scan|, and returns true.
// Returns false for bare regular register names (A, WA, XWA, etc.).
