...
DisI8086::DisI8086(const ValueFormatter::Plugins &plugins)
    : Disassembler(plugins, &_opt_fpu),
      ...
      _opt_repInsn(
              this, &DisI8086::setRepeatInsn, OPT_BOOL_REP_INSN, OPT_DESC_REP_INSN, &_opt_farInsn),
      _opt_farInsn(
...
Error DisI8086::searchCodes(DisInsn &insn, StrBuffer &out) const {
    ...
    opc = insn.readPrefixCodes(opc, _cpuSpec, _segInsn && !_gnuAs, _repInsn && !_gnuAs);
    if (insn.getError())
        return insn.getError();
...
Config::opcode_t DisInsn::readPrefixCodes(
    ...
    while (true) {
        if (!segInsn && isSegmentPrefix(cpuSpec, opc)) {
            if (_segment) {
