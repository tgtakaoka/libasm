...
Error AsmSuperH::parseOperand(StrScanner &scan, Operand &op) const {
    ...
    if (reg != REG_UNDEF) {
        ...
        if (isGpr(reg)) {
            ...
            switch (reg) {
            case REG_SR:   op.mode = M_SR;   break;
            case REG_GBR:  op.mode = M_GBR;  break;
            case REG_VBR:  op.mode = M_VBR;  break;
            case REG_MACH: op.mode = M_MACH; break;
            case REG_MACL: op.mode = M_MACL; break;
            case REG_PR:   op.mode = M_PR;   break;
            default:
                return op.setError(s, REGISTER_NOT_ALLOWED);
            }
        }
