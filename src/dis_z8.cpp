...
Error DisZ8::decodeIndexed(DisInsn &insn, StrBuffer &out, uint8_t opr1) {
    const auto dst = insn.dst();
    const auto src = insn.src();
    const auto pair = !(dst == M_X || src == M_X);
    uint16_t base16;
    if (dst == M_XL || src == M_XL) {
        base16 = insn.readUint16Le();
    } else {
...
Error DisZ8::decodePostByte(DisInsn &insn, StrBuffer &out) {
    ...
    if (dst == M_DA || src == M_DA) {  // P4: LDC, LDE
        ...
        if (dst == M_DA) {
            if (decodeAbsolute(insn, out, ENDIAN_LITTLE))
                return getError();
        } else {
    ...
    if (dst == M_XL || src == M_XL || dst == M_XS || src == M_XS)  // P1: LDC, LDE
        return decodeIndexed(insn, out, post & ~1);
    if (dst == M_RA) {  // P1: BTJRF, BTJRT
        if (decodeRelative(insn, out))
            return getError();
        out.comma();
        outWorkReg(out, post >> 4);
...
Error DisZ8::decodeImpl(DisMemory &memory, Insn &_insn, StrBuffer &out) {
    ...
    if (insn.postFormat() != PF_NONE)
        return decodePostByte(insn, out);
    ...
    if (dst == M_X || src == M_X) {
        const auto opr1 = insn.readByte();
        return decodeIndexed(insn, out, opr1);
    }
    if (dst == M_Irr || src == M_Irr)
        return decodeIndirectRegPair(insn, out);
    if (DisInsn::operandInOpCode(opCode))
        return decodeInOpCode(insn, out);
    if (src == M_NONE)
        return decodeOperand(insn, out, dst);
    return decodeTwoOperands(insn, out);
}
