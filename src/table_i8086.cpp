...
static constexpr Entry TABLE_D9[] PROGMEM = {
    ...
    E1(040,  TEXT_FLDENV,  SZ_NONE,  M_WMOD, P_OMOD),
    E1(050,  TEXT_FLDCW,   SZ_NONE,  M_WMOD, P_OMOD),
...
static constexpr Entry TABLE_DD[] PROGMEM = {
    ...
    E1(040,  TEXT_FRSTOR, SZ_NONE,  M_WMOD, P_OMOD),
...
Error TableI8086::searchName(const CpuSpec &cpuSpec, AsmInsn &insn) const {
    if (cpuSpec.fpu != FPU_NONE) {
        ...
        if (insn.getError() == UNKNOWN_INSTRUCTION) {
            ...
            if (strncasecmp_P(insn.name(), PSTR("FN"), 2) == 0) {
                char save[insn.nameBuffer().len() + 1];
                strcpy(save, insn.name());
                insn.nameBuffer().reset().letter('F').text(save + 2);
                fpu(cpuSpec.fpu)->searchName(insn, acceptModes);
                ...
                insn.nameBuffer().reset().text(save);
            }
...
bool TableI8086::isPrefix(const CpuSpec &cpuSpec, Config::opcode_t code) const {
