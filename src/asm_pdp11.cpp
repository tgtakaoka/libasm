...
struct Pdp11SymbolParser final : PrefixSymbolParser, Singleton<Pdp11SymbolParser> {
    Pdp11SymbolParser() : PrefixSymbolParser(PSTR_UNDER_DOT_DOLLAR) {}
...
Error to_float16(ValueStack &stack, ParserContext &context) {
...
RegName AsmPdp11::parseRegExpr(StrScanner &scan, Operand &op, char delim) const {
    ...
    if (p.expect('%')) {
        ErrorAt error;
        const auto regno = parseInteger(p, error, delim);
