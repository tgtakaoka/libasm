...
Error AsmI8096::parseOperand(StrScanner &scan, Operand &op) const {
    ...
    if (parseIndirect(p, op, p, true) != NOT_AN_EXPECTED) {
        if (op.hasError())
            return op.getError();
        scan = p;
        op.val.setUnsigned(0);  // zero displacement
        return OK;
    }
...
void AsmI8096::emitIndirect(AsmInsn &insn, const Operand &op) const {
    const auto regno = op.regno.getUnsigned();
    if (!isWreg(regno))
        insn.setErrorIf(op, OPERAND_NOT_ALIGNED);
    if (ioAddress(regno))
        insn.setErrorIf(op.regAt, ILLEGAL_REGISTER);
    insn.emitOperand8(regno);
}

void AsmI8096::emitIndexed(AsmInsn &insn, const Operand &op) const {
    const auto regno = op.regno.getUnsigned();
    if (!isWreg(regno))
        insn.setErrorIf(op, OPERAND_NOT_ALIGNED);
    if (ioAddress(regno))
...
void AsmI8096::emitAbsolute(AsmInsn &insn, const Operand &op) const {
...
void AsmI8096::emitOperand(AsmInsn &insn, AddrMode mode, const Operand &op) const {
    ...
    case M_WIDX:
        if (op.mode == M_ADDR) {
            if (mode == M_WIDX && !isWreg(op.val.getUnsigned()))
                insn.setErrorIf(op, OPERAND_NOT_ALIGNED);
            emitAbsolute(insn, op);
        } else {
            emitIndexed(insn, op);
