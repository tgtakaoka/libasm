...
struct Entry final : entry::Base<Config::opcode_t> {
    struct Flags final {
        ...
        static constexpr Flags create(CodeFormat cf, AddrMode dst, AddrMode src, AddrMode ext,
                OprPos dpos, OprPos spos, OprPos epos, OprSize size, bool needSize = false) {
            return Flags{mode(dst, src, ext), attr(dpos, spos, epos, size, cf, false, needSize)};
        }

        static constexpr Flags strInst(
                CodeFormat cf, AddrMode dst, AddrMode src, OprSize size, bool needSize = false) {
            return Flags{
                    mode(dst, src, M_NONE), attr(P_NONE, P_NONE, P_NONE, size, cf, true, needSize)};
        }
    ...
    private:
        ...
        static constexpr auto needSize_bp = 13;
