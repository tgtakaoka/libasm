...
bool acceptMode(AddrMode opr, AddrMode table) {
    if (opr == table)
        return true;
    if (opr == M_REG)
        return table == M_DREG || table == M_SREG || table == M_LREG || table == M_BREG ||
               table == M_INDIR || table == M_XREG;
    if (opr == M_DADDR)
        return table == M_IMM16 || table == M_BDISP || table == M_JADDR || table == M_BCOND ||
               table == M_SHCNT || table == M_BIT0;
    if (opr == M_NONE)
...
bool acceptModes(AsmInsn &insn, const Entry *entry) {
    ...
    return acceptMode(insn.srcOp.mode, flags.src()) && acceptMode(insn.dstOp.mode, flags.dst());
}
