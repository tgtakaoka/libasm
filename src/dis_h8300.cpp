...
void DisH8300::decodeAbsolute(DisInsn &insn, StrBuffer &out, AddrMode mode, OprPos pos) const {
    ...
    if (label) {
        ...
        if (mode == M_ABS8 || advancedMode() ||
                (mode == M_ABS16 && defaultsToAbs8(addr)))
...
RegName DisH8300::decodeOprAddrReg(
    ...
    return hasReg32() ? decodeOprReg32(insn, out, mode, pos)
                      : decodeOprReg16(insn, out, mode, pos);
}
...
void DisH8300::decodeOperand(DisInsn &insn, StrBuffer &out, AddrMode mode, OprPos pos) const {
    ...
    case M_RLIST: {
        ...
        const int first =
                (insn.dst() == M_RLIST) ? regno - static_cast<int>(count - 1) : regno;
        const bool ok =
                (count == 2 && first >= 0 && (first & 1) == 0) ||
