...
void DisZ80::decodeRelative(DisInsn &insn, StrBuffer &out, AddrMode mode) const {
    ...
    const uint32_t base = insn.address() + insn.length();
    const uint32_t target = base + delta;
...
void DisZ80::decodeOperand(DisInsn &insn, StrBuffer &out, AddrMode mode) const {
    ...
    case M_IDX:
        ...
        break;
    case M_IDX16:
        decodeLongIndex(insn, out, decodeIndexReg(insn.prefix()));
        break;
    case M_MPTR:
    ...
    case R_DXY:
    case R_SXY: {
        printf("@@ mode=%d prefix=%04X out=%s\n", mode, insn.prefix(), out.str());
