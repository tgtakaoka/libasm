...
DisZ80::DisZ80(const ValueFormatter::Plugins &plugins)
    : Disassembler(plugins, &_opt_extmode),
      Config(TABLE),
      _opt_extmode(this, &DisZ80::setExtendedMode, OPT_BOOL_EXTMODE, OPT_DESC_EXTMODE) {
    reset();
}
...
Error DisZ80::setExtendedMode(bool enable) {
    _extmode = enable;
    return OK;
}
...
Error DisZ80::decodeImpl(DisMemory &memory, Insn &_insn, StrBuffer &out) const {
    ...
    if (false && z380())
        printf("@@ search: opc=%02X prefix=%X\n", insn.opCode(), insn.prefix());
    if (searchOpCode(cpuType(), insn, out))
        return _insn.setError(insn);
    if (false && z380())
        printf("@@ found: %s opc=%02X prefix=%X dst=%d src=%d\n", insn.name(), insn.opCode(),
                insn.prefix(), insn.dst(), insn.src());
