...
Error AsmZ8::setRegisterPointer(uint8_t rp) {
    if (rp & 0x0F) {
        setError(ILLEGAL_CONSTANT);
    } else {
        _register_pointer = rp;
    }
    return OK;
}
...
Error AsmZ8::parseOperand(Operand &op) {
    ...
    if (indir) {
        if (op.val >= 0x100) return setError(OVERFLOW_RANGE);
        if (!regAddr && (op.val & 0xF0) == _register_pointer) {
    ...
    if ((op.val & 0xF0) == _register_pointer) {
        op.mode = M_w;
        op.reg = _regs.decodeRegNum(op.val & 0xF);
        return OK;
    }
