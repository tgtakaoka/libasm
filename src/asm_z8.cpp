...
Error AsmZ8::assumeRp(
    ...
    bool (AsmZ8::*set)(int16_t)) {
    const size_t length = strlen_P(name);
    const char *p = _parser.scanSymbol(scan);
    if (scan + length == p && strncasecmp_P(scan, name, length) == 0) {
        p = skipSpaces(p);
        if (*p == ':') {
...
Error AsmZ8::processPseudo(const char *scan, InsnZ8 &insn) {
    ...
    if (strcasecmp_P(insn.name(), TEXT_ASSUME) == 0) {
        if (assumeRp(p, TEXT_RP, &AsmZ8::setRegPointer) == 0)
            return OK;
        if (TableZ8.isSuper8()) {
            if (assumeRp(p, TEXT_RP0, &AsmZ8::setRegPointer0) == OK)
                return OK;
            if (assumeRp(p, TEXT_RP1, &AsmZ8::setRegPointer1) == OK)
                return OK;
        }
