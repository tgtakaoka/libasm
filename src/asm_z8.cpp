...
Error AsmZ8::processPseudo(InsnZ8 &insn, const char *line) {
    ...
    if (setRp(insn, line, "SETRP", &AsmZ8::setRegPointer) == OK)
        return OK;
    if (TableZ8.isSuper8()) {
        if (setRp(insn, line, "SETRP0", &AsmZ8::setRegPointer0) == OK)
            return OK;
        if (setRp(insn, line, "SETRP1", &AsmZ8::setRegPointer1) == OK)
            return OK;
    }
    if (strcasecmp(insn.name(), "ASSUME") == 0) {
        if (assumeRp(line, "RP", &AsmZ8::setRegPointer) == 0)
            return OK;
        if (TableZ8.isSuper8()) {
            if (assumeRp(line, "RP0", &AsmZ8::setRegPointer0) == OK)
                return OK;
            if (assumeRp(line, "RP1", &AsmZ8::setRegPointer1) == OK)
                return OK;
        }
