...
Error AsmMos6502::parseZeroOne(const char *scan, bool &val) {
    const char *p = skipSpaces(scan);
    if (*p == ':') {
        uint32_t val32 = parseExpr32(p + 1);
        if (getError() == OK) {
...
Error AsmMos6502::processPseudo(const char *scan, InsnMos6502 &insn) {
    ...
    if (strcasecmp_P(insn.name(), TEXT_ASSUME) == 0) {
        ...
        if (strncasecmp_P(p, TEXT_M, end - p) == 0)
            return parseZeroOne(end, _long_acc);
        if (strncasecmp_P(p, TEXT_X, end - p) == 0)
