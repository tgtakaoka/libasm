...
uint8_t ieee10(double value, uint8_t dest[10]) {
    union {
        double float64;
        uint8_t buffer[8];
    };
    ...
    const auto sign = buffer[7] & 0x80;
    uint16_t exponent = (buffer[6] >> 4) + (((uint16_t)buffer[7] & 0x7F) << 4);
    const auto denormal = exponent == 0;
    exponent = (exponent == 2047) ? 32767 : exponent + (16383 - 1023);
    buffer[6] &= 0x0F;
    if (!denormal)
        buffer[6] |= 0x10;
    for (auto i = 7; i >= 2; --i)
        dest[i] = ((buffer[i - 1] & 0x1F) << 3) | ((buffer[i - 2] & 0xE0) >> 5);
...
void Assembler::generateFloat80Le(double value, Insn &insn) const {
    uint8_t dest[10];
    const auto sign = ieee10(value, dest);
    if (isinf(value)) {
        insn.emitUint64Le(0);
        insn.emitUint16Le(sign ? 0xFFFF : 0x7FFF);
    } else if (isnan(value)) {
        insn.emitUint64Le(INT64_MIN);
        insn.emitUint16Le(sign ? 0xFFFF : 0x7FFF);
    } else {
        for (auto i = 0; i < 10; ++i)
            insn.emitByte(dest[i]);
    }
}
...
void Assembler::generateFloat96Be(double value, Insn &insn, uint8_t pos) const {
    uint8_t dest[10];
    const auto sign = ieee10(value, dest) ? 0x8000 : 0x0000;
    if (isinf(value)) {
        ...
        insn.emitByte(dest[9], pos++);
        insn.emitByte(dest[8], pos++);
        insn.emitUint16Be(0, pos);
        pos += 2;
        for (auto i = 0; i < 8; ++i) {
            insn.emitByte(dest[7 - i], pos++);
        }
    }
}
...
void Assembler::generatePackedBcd96Be(double value, Insn &insn, uint8_t pos) const {
    uint8_t dest[10];
    auto sign = ieee10(value, dest) ? 0x8000 : 0x0000;
    if (sign)
        value = -value;
    if (isinf(value)) {
        insn.emitUint16Be(sign | 0x2000, pos);  // INF??
        insn.emitUint16Be(0, pos += 2);
        insn.emitUint64Be(0, pos += 2);
    } else if (isnan(value)) {
        insn.emitUint16Be(sign | 0x1000, pos);  // NAN??
        insn.emitUint16Be(0, pos += 2);
        insn.emitUint64Be(0, pos += 2);
    } else {
        ...
        uint64_t mantissa = value * pow(10.0, 16 - exponent);
        if (exponent < 0) {
            sign |= 0x4000;
            exponent = -exponent;
        }
        ...
        insn.emitUint64Be(convert2pbcd(mantissa, 16), pos += 2);
    }
}
