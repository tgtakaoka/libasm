...
Error Assembler::defineFloatConstant(StrScanner &scan, Insn &insn, uint8_t dataType) {
    ...
    do {
        ...
        auto integer = true;
        if (exprErr.isOK() && (*p != '.' && *p != 'e' && *p != 'E')) {
            integer = true;
        } else {
            exprErr.setOK();
            errno = 0;
            char *end;
            val64 = strtoull(p.str(), &end, 10);
            const auto fpnum = (*end == '.' || *end == 'e' || *end == 'E');
            integer = (end != p.str() && errno != ERANGE && !fpnum);
            if (integer) {
        ...
        if (!exprErr.hasError()) {
            ...
            case DATA_FLOAT32_LONG:
                if (integer && val64 <= UINT32_MAX) {
                    ...
                    error.setErrorIf(at, big ? insn.emitUint32Be(val64) : insn.emitUint32Le(val64));
                    break;
                }
                /* Fall-through */
            case DATA_FLOAT32:
                error.setErrorIf(at, big ? insn.emitFloat32Be(value) : insn.emitFloat32Le(value));
                break;
            case DATA_FLOAT64_QUAD:
                if (integer) {
                    if (negative)
                        val64 = -static_cast<int64_t>(val64);
                    error.setErrorIf(at, big ? insn.emitUint64Be(val64) : insn.emitUint64Le(val64));
                    break;
                }
                /* Fall-through */
            case DATA_FLOAT64:
                error.setErrorIf(at, big ? insn.emitFloat64Be(value) : insn.emitFloat64Le(value));
                break;
            ...
            case DATA_FLOAT96:
                if (big) {  // MC68881
                    const uint16_t sign = negative ? 0x8000 : 0;
                    if (isinf(value)) {
                        insn.emitUint16Be(sign | 0x7FFF);
                        insn.emitUint16Be(0);
                        insn.emitUint64Be(0);
                    } else if (isnan(value)) {
                        insn.emitUint16Be(sign | 0x7FFF);
                        insn.emitUint16Be(0);
                        insn.emitUint64Be(static_cast<uint64_t>(1) << 63);
                    } else {
                        const int16_t exponent = log2l(value);
                        const auto significand = value / powl(2.0, exponent);
                        insn.emitUint16Be(sign | (exponent + 16383));
                        insn.emitUint16Be(0);
                        insn.emitUint64Be(significand * powl(2.0, 63));
                    }
                }
                break;
            case DATA_PBCD96:
                if (big) {  // MC68881
                    ...
                    if (isinf(value)) {
                        ...
                        insn.emitUint16Be(sign | convert2pbcd(exponent, 3));
                        insn.emitUint16Be(value);
                        insn.emitUint64Be(convert2pbcd(value * powl(10.0, 16), 16));
                    }
                }
                break;
            default:
